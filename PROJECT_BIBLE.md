...
## Güncel geliştirme checkpoint'i — Issue #64 r33 Mac ve Windows teknik kapıları geçti; final docs/CI ve USB bekliyor

Epic #10'un accepted custom-PC request → immutable quote/BOM → exact serialized reservation dilimi feature `c7d38845ffccb5ae6e5365e580c238d70f8dac95`, tree `615c9c4398f6a0be16c3a693dd812aa3f5541291` ile draft [PR #65](https://github.com/cixanla/PC-Shop-Empire-3D/pull/65)'e ulaştı. Exact müşteri/ziyaret/consultation provenance'ına bağlı graphics-first request, deterministic on-satırlı compatible quote ve motherboard/CPU/DIMM/M.2/cooler/GPU/PSU/ATX24/EPS12V/PCIe 6+2 için tek managed Inventory operation altında atomik exact-item reservation seti oluşturur. Claim/operation/access/revision/payload çapraz doğrulaması, exact replay, publication-recovery, conflict/drift/direct-release no-mutation kapıları testlidir. GarageGraybox r33 keyboard/mouse ve gamepad ile consultation→accepted request→visible quote/BOM→10 exact reservations akışını gösterir; pause-toggle frame'i stale hareket/bakış işlemez. Mac ve Windows EditMode `647/647`, PlayMode `59/59`; Universal Mac `329396456` bayt ve Apple M1/Metal exact r33 smoke, Windows x64 IL2CPP `1326137709` report baytı ve Intel Iris Xe/Direct3D 11.0 exact r33 smoke başarılıdır. Feature Guard [32698054990](https://github.com/cixanla/PC-Shop-Empire-3D/actions/runs/32698054990) geçti; iki masaüstü bağlantısı doğrulanmış build'lere çözülür. Kısa görünür Mac turu hareket ve pause/resume no-lurch kapısını gözledi. Kullanıcı USB kablosunun bağlı olduğunu bildirmiştir; ancak son salt-okunur denetimde beklenen `/Volumes/cixanla/CIXANLA` mount'u görünmemiştir ve yanlış volume'a yazılmamıştır. İki tam fiziksel readback ve final metadata/Guard yapılmadan [Issue #64](https://github.com/cixanla/PC-Shop-Empire-3D/issues/64) ile Roadmap açık/In Progress kalır. Reservation consume/build order, fiziksel montaj tamamlama, power-on/POST/OS, benchmark/QA, paketleme/teslim, final settlement, Save/Guardian ve Steam sonraki bağımlı kapılardır.
...
| 3 | PC toplama teknik prototipi | Devam ediyor; anakart/fastener, CPU, DIMM, M.2, cooler, GPU, PSU, ATX24, EPS12V ve PCIe/GPU 6+2 fiziksel authority akışları ile accepted request→immutable quote/BOM→10 exact serialized reservation sınırı uygulandı. Güncel kaynak Mac ve Windows test/build/native kapılarından geçti; Issue #63 ve #64 fiziksel USB/final metadata kapanışları açık |
...
| Son test/build | Issue #64 Edit Mode `647/647`, gerçek Input System Play Mode `59/59`, failed/skipped/inconclusive `0`; Universal macOS build `329396456` bayt ve Apple M1/Metal exact r33 smoke; Windows x64 IL2CPP build `1326137709` report baytı ve Intel Iris Xe/Direct3D 11.0 feature level 11.1 exact r33 smoke geçti. Scene `2854602` bayt, SHA-256 `7fc63ba4686db17f5ca7800bf2421a526df591659dc84c201439f153416ff338` |

Önceki zaman/olay Core commit'i `8af2ad3d05906839c4b607e4958650e723060465`, iş birliği/devir checkpoint'i `2ee421193833111f76c85dabb33910240c36db03` ve Issue #50–#62 kapanış checkpointleri tarihsel olarak korunur. Issue #63'ün Mac+Windows teknik checkpoint'i ADR-0041'de, Issue #64'ün iki-platform teknik checkpoint'i ADR-0042'de ve ilgili tarihli Evidence belgelerinde kayıtlıdır. Fiziksel USB ve final metadata/Guard kapıları tamamlanmadan bu iki Issue kapalı sayılmaz.

## 16. Sıradaki uygulama sırası

1. Issue #64 yaşayan belge setini commit/push et, draft PR #65 üzerinde final Repository Guard'ı doğrula; Issue #63'ün artık başarılı olan Windows r32 kanıtını GitHub kaydına ekle.
2. USB bağlı değilken yalnız yerel değişmez source-plus-evidence staging paketlerini hazırla ve kendi içlerinde iki tam readback ile doğrula; `/Volumes` sorgulama veya yazma yapma.
3. Kullanıcı doğru USB'nin bağlandığını açıkça bildirdiğinde önce volume/milestone zincirini salt-okunur doğrula; Issue #63 ve #64 paketlerini ayrı çakışmayan `.incoming-*` hedeflerde iki tam manifest/Git/evidence readback ve atomik final adlandırmayla kapat, ardından metadata/Guard/Issue/Roadmap durumlarını tamamla.
4. Sonraki bounded oyuncu-görünür paket olarak exact reservation setini immutable build order'a tüketme/devretme akışını uygula; sonra fiziksel build-kit tamamlama, power-on/POST, OS/driver, benchmark/QA, paketleme/teslimat ve final settlement zincirini bağımlılık sırasıyla ilerlet. Mevcut graybox'ı final art sayma ve Guardian'ı gameplay authority yapma.
...
5. Edit Mode 647/647 ve Play Mode 59/59 baseline testlerini doğrula.
