...
**Son kapsam güncellemesi:** 24 Ağustos 2026
...
## Güncel geliştirme checkpoint'i — Issue #63 r32 teknik dilimi Mac üzerinde doğrulandı; Windows/USB kapanışı bekliyor

Epic #10'un tek PCIe/GPU 8-pin 6+2 güç kablosu dilimi feature `ea1e51f862d4094936c03bccf9fbfaee7bb7d12b` ile private `main`e ulaştı. Tek canonical serialized kablo; iki distinct typed/keyed endpoint, üç stable ordered waypoint, capacity-one `GpuPowerCableRoute` ve retained PSU + secured motherboard + retained GPU lineage'ını atomik on-container Inventory claim'i içinde kullanır. GarageGraybox r32; aynı Unity instance ve stable ItemId ile pickup→keyed preview→authored PSU-to-GPU route→exact unroute→world drop/recovery akışını, ATX24/EPS12V'den izole authority'yi ve PSU/GPU/motherboard dependent-detach ile generic-bypass fail-closed kapılarını görünür biçimde taşır. Görsel düzeltme `d655f1a5aab0c882cf40702472ec1b8ad44747ad` ile PSU ucunu monolitik 8-pin bırakırken GPU ucunu ayrı 6-pin + 2-pin housing, keyed latch, retention clip ve `6`/`2` etiketleriyle fiziksel olarak ayırdı; yeni dekoratif parçalar collider/raycast authority değildir. EditMode `626/626`, gerçek Input System PlayMode `53/53`, Universal macOS `329334656` bayt ve aktif Apple M1/Metal 1280×720 exact PCIe/GPU smoke geçti. Feature Guard [32676069923](https://github.com/cixanla/PC-Shop-Empire-3D/actions/runs/32676069923), Windows IL2CPP gate Guard [32676154473](https://github.com/cixanla/PC-Shop-Empire-3D/actions/runs/32676154473) ve 6+2 görsel düzeltme Guard [32677267023](https://github.com/cixanla/PC-Shop-Empire-3D/actions/runs/32677267023) başarılıdır. Windows clone build denemesi anında exact/temiz ve IL2CPP+C++ toolchain hazırdı; native build kaynak derlemesinden önce etkin Unity Editor lisansı bulunmadığı için exit `198` ile durmuştur. Doğru USB artık `/Volumes/cixanla/CIXANLA`, `90_BACKUPS/PCShopEmpire3D` ve Issue #62 milestone zinciriyle doğrulanmıştır; ancak Windows native kanıtı ve final source/docs kimliği oluşmadan Issue #63 final milestone'u yazılmamıştır. [Issue #63](https://github.com/cixanla/PC-Shop-Empire-3D/issues/63) ve Roadmap bu iki gerçek dış kapı geçene kadar açık/In Progress kalır; electrical power-on, wattage/headroom, POST/BIOS/OS ve completed benchmark kapsam dışıdır.
...
| 3 | PC toplama teknik prototipi | Devam ediyor; anakart/fastener, CPU, DIMM, M.2, cooler, GPU, PSU, ATX24, EPS12V ve PCIe/GPU 6+2 fiziksel authority akışları uygulanıp Mac test/build/runtime kapılarından geçti; Issue #63 Windows IL2CPP/DirectX ve USB kapanışı bekliyor |
...
| Issue #63 teknik checkpoint'i | Feature `ea1e51f862d4094936c03bccf9fbfaee7bb7d12b`, tree `ecc32279a8e17e8179114a9b6cfcfe4737827601`; Windows gate `cdfe9d6a3bed20a6529fb045f69d7394b3b147c8`; explicit GPU-side 6+2 visual fix `d655f1a5aab0c882cf40702472ec1b8ad44747ad`, tree `c3fff116317db7e3388e0faf04e38a7ffaa7ce77`; Guard `32676069923` + `32676154473` + `32677267023` başarılı; gameplay/domain/görsel acceptance Mac üzerinde kanıtlandı, Windows Editor lisansı ve final USB readback bekliyor; Issue/Roadmap açık/In Progress |
| Son test/build | Issue #63 sonrası Edit Mode `626/626` (`issue63-r32-6plus2-surgical-editmode-final.xml`), gerçek Input System Play Mode `53/53` (`issue63-r32-6plus2-surgical-playmode-final.xml`), failed/skipped/inconclusive `0`; Universal macOS build `329334656` bayt ve aktif Apple M1/Metal makinesinde 1280×720 `garage-pcie-gpu-power-cable-routing-r32-v1` readiness + exact PCIe/GPU smoke geçti; scene `2854602` bayt, SHA-256 `7fc63ba4686db17f5ca7800bf2421a526df591659dc84c201439f153416ff338`; Windows IL2CPP henüz lisans engeli nedeniyle kanıtlanmadı |

Önceki zaman/olay Core commit'i `8af2ad3d05906839c4b607e4958650e723060465`, iş birliği/devir checkpoint'i `2ee421193833111f76c85dabb33910240c36db03` ve Issue #50–#62 kapanış checkpointleri tarihsel olarak korunur. Issue #63'ün doğrulanmış Mac teknik checkpoint'i ADR-0041 ve tarihli evidence belgesinde kayıtlıdır; Windows lisans/IL2CPP/DirectX ve fiziksel USB kapıları tamamlanmadan Issue kapanmış sayılmaz.

## 16. Sıradaki uygulama sırası

1. Windows makinede Unity Personal Editor lisansını kullanıcı hesabıyla etkinleştir; clone'u final Issue #63 source/docs head'ine fast-forward et, StrictMode IL2CPP x64 build ve DirectX r32 smoke'u çalıştır. Lisans etkinleşmeden build sonucu uydurma veya Mono ile ikame etme.
2. Doğru USB mount'u `90_BACKUPS/PCShopEmpire3D` ve Issue #62 zinciriyle şimdiden doğrulanmıştır. Windows gate geçince final source-plus-evidence staging→çakışmayan `.incoming-*`→iki tam manifest/Git/evidence readback→atomik final adlandırma kapanışını tamamla; sonra Issue #63 ve Roadmap'ı kapat.
3. #63 kapandıktan sonra kablo ailesi eklemeyi durdur ve tek gerçek özel-PC işi için accepted request→immutable quote/BOM→exact serialized Inventory reservation bounded paketini başlat.
4. Save v1, physical build-kit, power-on/POST, OS/driver, benchmark/QA, paketleme/teslimat ve final settlement zincirini bağımlılık sırasıyla ilerlet; mevcut graybox'ı final art sayma ve Guardian'ı gameplay authority yapma.
