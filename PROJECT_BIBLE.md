...
Epic #10'un tek ATX PS/2 güç kaynağı seating + dört arka vida retention dilimi tamamlandı. Feature `f998d7d` ve authored-clearance fix'i `b6c3ff8`; tek canonical serialized PSU item'ını atomik yedi-container Inventory claim'i, stable chassis-owned bay/rear-mount/four-fastener topology'si, reversible seat/retain/unretain/remove authority'si, exact replay, alternate-order izolasyonu, same-instance recovery ve gerçek keyboard/gamepad dynamic prompt'larıyla GarageGraybox r29'a taşır. Production clearance yalnız gerçek `ChassisBack`, `ChassisLeftRail`, `ChassisRightRail` ve `MotherboardTray` collider'larını kullanır; destek yüzeyleri blocker değildir ve kablolama ayrı kaldığı için cable blocker listesi boştur. Final doğrulama EditMode `577/577`, PlayMode `47/47`, Universal macOS `328937592` bayt ve aktif Apple Silicon/Metal 1280×720 exact PSU smoke'tur. Feature/fix/source-docs Guard'ları `32606958882` + `32607437408` + `32607886160` başarılıdır; source/docs `4939a04`, acceptance `20/20`, Issue #60 `CLOSED/COMPLETED` ve Roadmap `Done`dur. Parent Epic #10 açık/In Progress kalır. Exact yerel staging 775/775 readback ve 770/770 Git eşliğiyle hazırdır; macOS harici fiziksel USB göstermediğinden yanlış volume'a yazılmamış ve fiziksel readback iddiası kurulmamıştır.
...
1. Ardıl Epic #10 child'ını kablo konektörleri ve fiziksel routing için ayrıca sınırla; electrical power-on, POST/BIOS/OS ve tamamlanmış benchmarkı aynı pakete gizlice ekleme.
2. Beklenen fiziksel USB gerçekten mount edildiğinde doğrulanmış Issue #60 local staging paketini çakışmayan `.incoming-*` + iki tam readback ile taşı; o zamana kadar yanlış volume'a yazma veya gameplay'i bloklama.
