...
**Son kapsam güncellemesi:** 22 Ağustos 2026
...
## Güncel geliştirme checkpoint'i — Issue #59

Epic #10'un tek PCIe x16 ekran kartı seating + slot-latch/rear-bracket retention diliminin kaynak, test, build, native runtime ve feature-CI kapıları tamamlandı. Feature `1b29ad2`, canonical Northstar A60 ProductId'sini koruyan ayrı serialized assembly item'ını atomik altı-container Inventory claim'i, stable slot/latch/bracket/fastener topology'si, reversible seat/retain/unretain/remove authority'si, exact replay/host gates, same-instance recovery ve gerçek keyboard/gamepad dynamic prompt'larıyla GarageGraybox r28'e taşır. Final doğrulama EditMode `548/548`, PlayMode `43/43`, Universal macOS `328781520` bayt, aktif Apple Silicon/Metal 1280×720 exact GPU smoke ve başarılı feature Guard `32599710154`'tür. Source/docs Guard ile Issue #59 acceptance/Roadmap kapanışı bu checkpointin final metadata adımında kaydedilecektir. Fiziksel USB kullanıcı yeniden bağlandığını bildirene kadar sorgulanmaz; Issue #58'in 717/717 staging'i korunur, Issue #59 için henüz fiziksel milestone/readback iddiası yoktur ve geliştirme GitHub üzerinden güvenle sürer.
...
| Son tamamlanmış USB milestone (Issue #57) | `2026-08-16_STAGE_B_DETERMINISTIC_SINGLE_M2_NVME_CAPTIVE_SCREW`; source/docs `6e0627e`, exact Git source + 4 final kanıt + source kaydı, 689/689 `19da758c…21b8` SHA-256 manifest/readback ve AppleDouble `0` |
...
| Issue #59 kaynak checkpoint'i | Feature `1b29ad29d6e4d8cc1ce09b0989a038a72297585c`, tree `e7b3d5d4ad13bf08f822570966660ab6c48e6a55`; [feature Guard 32599710154](https://github.com/cixanla/PC-Shop-Empire-3D/actions/runs/32599710154) başarılı; source/docs Guard ve acceptance `20/20`/Issue/Roadmap `Done` final metadata adımında kaydedilecek; fiziksel USB ertelendi ve Issue #59 için milestone/readback iddiası yok |
| Son test/build | Issue #59 sonrası Edit Mode `548/548` (`editmode-issue59-final.xml`), gerçek Input System Play Mode `43/43` (`playmode-issue59-final.xml`), failed/skipped/inconclusive `0`; Universal macOS build `328781520` bayt ve aktif Apple Silicon/Metal makinesinde 1280×720 `garage-gpu-rear-bracket-r28-v1` readiness + exact GPU smoke geçti; scene SHA-256 `14f405a657c8d9b5a1719be85d8be4c254fbf91983852170892dd00118dfaf5b` |

Önceki zaman/olay Core commit'i `8af2ad3d05906839c4b607e4958650e723060465`, iş birliği/devir checkpoint'i `2ee421193833111f76c85dabb33910240c36db03` ve Issue #50–#58 checkpointleri tarihsel olarak korunur. Issue #59 kaynak/test/build/runtime doğrulaması `Docs/ProjectBible/10_DEVAM_CHECKPOINT.md`, ADR-0037 ve tarihli evidence belgesinde kayıtlıdır.

## 16. Sıradaki uygulama sırası

1. Issue #59 source/docs Guard ve GitHub/Roadmap kapanış metadatasını tamamladıktan sonra ardıl Epic #10 child'ını ayrı issue ve acceptance sözleşmesiyle tek fiziksel PSU seating/retention bileşenine sınırla; kablolama ve tam benchmarkı aynı pakette birleştirme.
2. Kullanıcı USB'nin yeniden bağlı olduğunu söylediğinde hazır Issue #58 staging paketini ve güncel Issue #59 GitHub checkpointini çakışmayan `.incoming-*` + tam readback disipliniyle fiziksel milestone'a taşı; o zamana kadar USB arama veya geliştirmeyi durdurma.
