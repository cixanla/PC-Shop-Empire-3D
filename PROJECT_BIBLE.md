...
## Güncel geliştirme checkpoint'i — Issue #66 r34 Mac ve Windows teknik kapıları geçti; source/docs ve fiziksel USB kapanışı bekliyor

Epic #10'un accepted quote/reservation → immutable build order → physical work ticket dilimi core feature `f9545605baff423f05615e7326902e24dc82aeeb`, current technical head `f8afd62326c74aff23fa10bb33ef79ecb9a656b6`, tree `69ea366cc49e99b653f5d02d9c0f238b4906de69` ile draft [PR #67](https://github.com/cixanla/PC-Shop-Empire-3D/pull/67)'ye ulaştı. Exact customer/request/quote/claim/workbench ve on line/item/reservation identity tek immutable BuildOrder + WorkTicket içinde donar; Inventory aynı managed set için bir terminal operation-keyed allocation receipt'i exactly-once yayınlar. On reservation ve serialized item canlı, yerinde ve değişmeden kalır; move/delete/release/consume, second allocation, orphan recovery ve mismatched replay fail-closed'dur. GarageGraybox r34 canonical workbench'te job identity, `10/10` ve `MONTAJA HAZIR • HENÜZ BAŞLAMADI` gösteren collider-safe physical ticket taşır. Range/focus/LOS/empty-hands/fresh Interact, pause/co-edge/competing-target ve gerçek keyboard/mouse/gamepad customer→workbench rotası testlidir; physical ticket/carry/cart aynı-frame Interact ownership'i deterministic, customer-reserved shelf stock istisnası bounded ve Quote/Assembly authority'leri izoledir. EditMode `661/661`, PlayMode `66/66`; Universal Mac `329478891` bayt ve Apple M1/Metal exact r34 smoke, clean Windows x64 IL2CPP `1328828053` report baytı ve Intel Iris Xe/Direct3D 11.0 feature level 11.1 exact r34 smoke başarılıdır. Current-source [Repository Guard 32721069982](https://github.com/cixanla/PC-Shop-Empire-3D/actions/runs/32721069982) geçti. ADR-0043 ve tarihli Evidence exact canonical `9/9` allowlist'i `/Users/cixanla/Developer/PCShopEmpire3D/TestResults/issue66-f8afd62` kaynağına bağlar. USB kimliği ve önceki milestone zinciri her fiziksel yazımdan hemen önce yeniden doğrulanacaktır; yanlış volume'a yazılmaz. Source/docs commit, yerel immutable staging, fiziksel USB iki tam readback, final metadata/Guard, acceptance `18/18`, Issue/Roadmap kapanışı ve PR merge tamamlanmadan [Issue #66](https://github.com/cixanla/PC-Shop-Empire-3D/issues/66) açık/In Progress kalır. Fiziksel component transfer/build-kit completion, montaj, power-on/POST/OS, benchmark/QA, paketleme/teslim, final settlement, Save/Guardian ve Steam sonraki bağımlı kapılardır.
...
| 3 | PC toplama teknik prototipi | Devam ediyor; anakart/fastener, CPU, DIMM, M.2, cooler, GPU, PSU, ATX24, EPS12V ve PCIe/GPU 6+2 fiziksel authority akışları ile accepted request→immutable quote/BOM→10 exact reservation→immutable build order/physical work ticket sınırı uygulandı. Güncel r34 kaynak Mac ve Windows test/build/native kapılarından geçti; Issue #63, #64 ve #66 fiziksel USB/final metadata kapanışları açık |
...
| Issue #66 teknik checkpoint'i | Core feature `f9545605baff423f05615e7326902e24dc82aeeb`, current head `f8afd62326c74aff23fa10bb33ef79ecb9a656b6`, tree `69ea366cc49e99b653f5d02d9c0f238b4906de69`; exact quote/reservation setinden immutable BuildOrder + WorkTicket, exactly-once allocation receipt ve canonical workbench physical ticket. Guard `32721069982`, draft PR #67; Mac+Windows full test/build/native r34 geçti. Source/docs ve fiziksel USB/final metadata bekliyor, Issue/Roadmap açık/In Progress |
| Son test/build | Issue #66 Edit Mode `661/661`, gerçek Input System Play Mode `66/66`, failed/skipped `0`; Universal macOS build `329478891` bayt ve Apple M1/Metal exact r34 smoke; Windows x64 IL2CPP build `1328828053` report baytı ve Intel Iris Xe/Direct3D 11.0 feature level 11.1 exact r34 smoke geçti. Scene `2910046` bayt, SHA-256 `005813677c28bdd2a2ae4f656c3c5ef9b2d04c786cd552bcb1746928b146cc0f` |

Önceki zaman/olay Core commit'i `8af2ad3d05906839c4b607e4958650e723060465`, iş birliği/devir checkpoint'i `2ee421193833111f76c85dabb33910240c36db03` ve Issue #50–#62 kapanış checkpointleri tarihsel olarak korunur. Issue #63, #64 ve #66 iki-platform teknik checkpointleri sırasıyla ADR-0041, ADR-0042 ve ADR-0043 ile ilgili tarihli Evidence belgelerinde kayıtlıdır. Fiziksel USB ve final metadata/Guard kapıları tamamlanmadan bu açık Issue'lar kapalı sayılmaz.

## 16. Sıradaki uygulama sırası

1. Issue #66 ADR/Evidence/Project Bible setini source/docs commitinde push et, draft PR #67 ve yeni Repository Guard'ı doğrula.
2. USB bağlı değilken `2026-08-24_STAGE_B_IMMUTABLE_CUSTOM_PC_WORK_ORDER_PHYSICAL_WORK_TICKET_HANDOFF` yerel immutable source-plus-evidence staging paketini hazırla ve kendi içinde tam manifest/Git/evidence readback ile doğrula; yanlış volume'a yazma.
3. Kullanıcı doğru USB'nin bağlandığını açıkça bildirdiğinde önce external physical disk, exact `/Volumes/cixanla/CIXANLA`, `90_BACKUPS/PCShopEmpire3D` ve önceki milestone zincirini salt-okunur doğrula. Yalnız collision-free `.incoming-*` hedefte ilk tam readback, atomik final adlandırma ve ikinci tam readback sonrasında metadata/Guard/acceptance `18/18`/Issue/Roadmap/PR kapanışını tamamla.
4. Sonraki bounded oyuncu-görünür paket olarak work order'a ayrılmış exact item setinin fiziksel component transfer/build-kit completion akışını tanımla; sonra montaj, power-on/POST, OS/driver, benchmark/QA, paketleme/teslimat ve final settlement zincirini bağımlılık sırasıyla ilerlet. Mevcut graybox'ı final art sayma ve Guardian'ı gameplay authority yapma.
...
5. Edit Mode 661/661 ve Play Mode 66/66 baseline testlerini doğrula.
