    ...
    public sealed class Atx24PowerCableAssemblyItemBinding : MonoBehaviour
    {
            ...
            int heldLayer)
        {
            ...
            if (domain.IsFailure)
            {
                ...
                {
                    return OperationResult.Fail(
                        Failure.FromCode("assembly-power-cable.safe-pose-rollback-failed"));
                }
                ...
                if (rollback.IsFailure)
                {
                    ...
                    OperationResult authorityRecovery = physicalRecovery.IsSuccess
                        ? Session.DropHeldAtx24PowerCableToWorld()
                        : OperationResult.Fail(
                            Failure.FromCode("assembly-power-cable.recovery-unavailable"));
                    ...
                    return physicalRecovery.IsFailure || authorityRecovery.IsFailure
                        ? OperationResult.Fail(Failure.FromCode(
                            "assembly-power-cable.physical-rollback-compensation-failed"))
                        : OperationResult.Fail(Failure.FromCode(
                            "assembly-power-cable.physical-rollback-failed"));
                }
