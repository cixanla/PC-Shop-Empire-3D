...
{
    public sealed class PcieGpuPowerCableRouteSolverTests
    {
        ...
        public void ModeOffReturnsBeforeContextAndPerformsZeroQueries()
        {
            ...
            PcieGpuPowerCableRouteEvaluation result =
                PcieGpuPowerCableRouteSolver.Evaluate(
                    false,
                    null,
                    null,
                    null,
            ...
            Assert.That(result.Status,
                Is.EqualTo(PcieGpuPowerCableRouteStatus.ModeDisabled));
        ...
        [Test]
        public void MotherboardPsuGraphicsCardAndOrientationGatesRunBeforePhysics()
        {
            Fixture fixture = CreateFixture();
            ...
            Assert.That(
                fixture.Evaluate(graphicsCardRetained: false).Status,
                Is.EqualTo(
        ...
        public void ExactDistanceLineOfSightTieFailsClosedInEitherOrder()
        {
            ...
            Collider blocker = CreateCube(
                "PcieGpuLosTieBlocker",
                new Vector3(0f, 0f, 0.5f),
                Vector3.one * 0.05f).GetComponent<Collider>();

            fixture.Physics.SetLineHits(
                new PcieGpuPowerCablePhysicsHit(fixture.Focus, 1f),
                new PcieGpuPowerCablePhysicsHit(blocker, 1f));
            PcieGpuPowerCableRouteEvaluation targetFirst = fixture.Evaluate();

            fixture.Physics.SetLineHits(
                new PcieGpuPowerCablePhysicsHit(blocker, 1f),
                new PcieGpuPowerCablePhysicsHit(fixture.Focus, 1f));
            PcieGpuPowerCableRouteEvaluation blockerFirst = fixture.Evaluate();

            Assert.That(targetFirst.Status,
                Is.EqualTo(PcieGpuPowerCableRouteStatus.LineOfSightBlocked));
            Assert.That(blockerFirst.Status, Is.EqualTo(targetFirst.Status));
            Assert.That(targetFirst.FailureCode,
                Is.EqualTo("assembly-pcie-gpu-cable.line-of-sight-blocked"));
        }
        ...
        public void ObstructionAndSaturatedOverlapQueriesFailClosed()
        {
            ...
            PcieGpuPowerCableRouteEvaluation obstructed = fixture.Evaluate();

            fixture.Physics.SetOverlaps();
            fixture.Physics.OverlapCountOverride =
                PcieGpuPowerCableRouteSolver.HitCapacity;
            PcieGpuPowerCableRouteEvaluation saturated = fixture.Evaluate();

            Assert.That(obstructed.Status,
                Is.EqualTo(PcieGpuPowerCableRouteStatus.RouteObstructed));
            Assert.That(saturated.Status,
                Is.EqualTo(PcieGpuPowerCableRouteStatus.QuerySaturated));
        }
        ...
        public void ExplicitAllowlistDoesNotIgnoreWholeHostHierarchy()
        {
            ...
            PcieGpuPowerCableRouteEvaluation allowed = fixture.Evaluate();
            ...
            PcieGpuPowerCableRouteEvaluation blocked = fixture.Evaluate();
        ...
        public void RoutedFocusUsesBoardEndpointWithoutOverlapQueries()
        {
            ...
            PcieGpuPowerCableRouteStatus status =
                PcieGpuPowerCableRouteSolver.EvaluateRoutedFocus(
                    true,
                    fixture.Origin,
                    fixture.Player,
                    fixture.Cable,
        ...
        private Fixture CreateFixture()
        {
            ...
            Transform cableRoot = CreateObject("PcieGpuCable").transform;
            ...
            Transform routeRoot = CreateObject("PcieGpuRoute").transform;
            ...
            Transform graphicsCardEndpoint = CreateChild(
                motherboardHost,
                "MotherboardCpu8",
                new Vector3(0f, 0f, 1f));
            graphicsCardEndpoint.rotation = Quaternion.Euler(0f, 90f, 0f);
            ...
            var physics = new FakeRoutePhysics();
            physics.SetLineHits(new PcieGpuPowerCablePhysicsHit(focus, 1f));
            return new Fixture(
                origin,
                player,
                cable,
        ...
        private readonly struct Fixture
        {
            ...
            public Transform GraphicsCardEndpoint { get; }
        ...
        private sealed class FakeRoutePhysics : IPcieGpuPowerCableRoutePhysics
        {
            private PcieGpuPowerCablePhysicsHit[] _lineHits =
                Array.Empty<PcieGpuPowerCablePhysicsHit>();
            ...
            public void SetLineHits(params PcieGpuPowerCablePhysicsHit[] hits)
            {
                _lineHits = hits ?? Array.Empty<PcieGpuPowerCablePhysicsHit>();
            }
