...
`Tools/verify-checkpoint-package.sh ... issue71` requires the exact 14-name evidence set plus full manifest path/size/SHA, exact Git-source, forbidden/cache/credential, secret-signature, symlink and AppleDouble gates. Its Issue #71 semantic gate additionally parses source/binary/procedure/runtime/task receipts, binds all commit/tree and procedure/promoted-artifact hashes, requires the `issue71-hardened-v2` policy, rejects Burst/native-link tokens, requires one exact CPU runtime marker and enforces final sibling `.incoming-*`/AppleDouble residue `0`. Omitting `issue71` intentionally falls back to generic canonical behavior and is not valid for Issue #71 closure.
