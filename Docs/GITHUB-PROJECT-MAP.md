...
| [#9](https://github.com/cixanla/PC-Shop-Empire-3D/issues/9) | [#52](https://github.com/cixanla/PC-Shop-Empire-3D/issues/52) — Physical checkout station and AwaitingCheckout-gated cash payment | Kaynak/test/build/runtime tamamlandı; stable station, range/focus/LOS/pause gate'i, RAF A bypass yasağı, exact customer/visit/basket/offer/item/reservation/action provenance ve gerçek Mouse/RT release-repress checkout→cash akışı; 352/352 EditMode, 24/24 PlayMode, `garage-physical-checkout-station-r21-v1 checkout-station=ok shelf-bypass-blocked=ok checkout-start=ok cash-payment=ok authority-isolated=ok customer-hidden=ok`; feature `92a0f7b`, [Guard `31892420515`](https://github.com/cixanla/PC-Shop-Empire-3D/actions/runs/31892420515); source/docs/USB/acceptance closure sürüyor |

## Sıradaki bounded kapı

Issue #52 için sıradaki kapı source/docs + Guard, exact USB snapshot/readback, acceptance `17/17`, Roadmap `Done` ve Issue close'dur. Ardından Epic #9 geniş kabulü ayrıca doğrulanır. Sonraki bounded geliştirme Epic #10 altında tek kasa/tek component/tek doğru slot ile ilk fiziksel PC assembly dilimidir; child issue henüz açılmamıştır.
