...
| [#9](https://github.com/cixanla/PC-Shop-Empire-3D/issues/9) | [#49](https://github.com/cixanla/PC-Shop-Empire-3D/issues/49) — Stale-safe Leave action and offer-declined exit | Feature/test/build/runtime tamamlandı; exact revalidation, `Browsing → Exiting`, stable `OfferDeclined`, commerce no-mutation ve Browse→Exit NavMesh; 298/298 EditMode, 22/22 PlayMode, `garage-leave-action-r18-v1 leave-action=ok stale-leave-blocked=ok`; source/docs/CI/USB kapanışı sürüyor |
