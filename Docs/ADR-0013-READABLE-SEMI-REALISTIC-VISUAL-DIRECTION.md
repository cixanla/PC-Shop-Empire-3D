...
## 31 Ağustos 2026 uygulama addendum'u — Issue #115

Issue #115, r56 sunum proof'unun açık bıraktığı scene-authority borcunu bounded biçimde kapatır. Legacy `StarterShelf` hierarchy kaldırılır; `AuthoritativeRetailShelfA` exact tek physical retail placement/inventory authority olarak `5` child collider, tek `PlacementSurface` ve tek shelf `InventoryPlacementZone` taşır. Presentation projection ikinci authority üretmez; customer approach/browse/checkout/fulfilled-exit, reservation/payment/receipt, stable identity ve input owner değişmez.

r56→r57 authored MeshRenderer etkisi `-16`, light/camera etkisi `0/0`dır. Retail runtime `486/462`, Assembly regression runtime `477/468`; accepted r56/r55 material/light/composition dili korunur. Apple M1/Metal üç retail capture glare `0` ve minimum contrast `1.348` verir; native marker `shelf-authority=single legacy-starter-shelf=absent` ile scene contract'ı bağlar.

Bu addendum Mac teknik kanıtını kaydeder; fiziksel Windows x64 IL2CPP/D3D11 Intel Iris Xe, USB checkpoint/readback ve physical-human HID/endurance kapıları henüz geçilmiş sayılmaz. UTM release donanımının yerine geçmez. Draft PR #118 ve Issue #115 bu yüzden açık/In Progress kalır. Exact kanıt [Retail Shelf Authority Consolidation checkpoint](Evidence/RETAIL-SHELF-AUTHORITY-CONSOLIDATION-CHECKPOINT-2026-08-31.md) dosyasındadır.
