...
Stage A ve private GitHub güvenlik/devir temeli tamamlandı. Oynanabilir garaj, Catalog + Inventory, atomik purchase-order receiving, görünür Receiving→eller→RAF A projection, idempotent koli açma, authoritative fiyat, customer basket reservation ve immutable checkout price snapshot hazırdır: Edit Mode 233/233, Play Mode 17/17 geçti; Universal Mac build/runtime smoke başarılıdır. Bir sonraki bounded iş exact checkout reservation'ını atomik tüketip serialized item için satış/fulfilled sonucu üretmektir; ödeme/Economy ledger ve Save ayrı kalır.
