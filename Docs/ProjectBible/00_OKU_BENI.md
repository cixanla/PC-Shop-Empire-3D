...
Stage A ve private GitHub güvenlik/devir temeli tamamlandı. Oynanabilir garaj, Catalog + Inventory, atomik purchase-order receiving, görünür Receiving→eller→RAF A projection, idempotent fiziksel koli açma, authoritative RAF A fiyatı ve customer basket serialized reservation hazırdır: Edit Mode 220/220, Play Mode 17/17 geçti; Universal Mac build/runtime smoke başarılıdır. Bir sonraki bounded iş basket line + exact offer + reservation'ı doğrulayıp fiyatı checkout başlangıcında immutable snapshot olarak donduran transaction sözleşmesidir; reservation consume ve ödeme/ledger bunu izler.
