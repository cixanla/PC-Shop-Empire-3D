...
**Branch:** `feature/issue87-pcie-gpu-power-cable-build-kit-handoff`
...
### 26 Ağustos 2026 üstün gelen güncel durum — Issue #87 PCIe/GPU Power-Cable BuildKit

- Exact reserved `PowerCable` yalnız `ModularPcie8PinPsuToGraphicsCard` family ve tam line/product/item/reservation lineage ile seçilir; staged motherboard+CPU+DDR5+M.2+processor-cooler+graphics-card+power-supply+ATX24+EPS12V sonrasında ayrı stable operation/capacity-one PCIe/GPU slotuyla WorldFloor → ActorHands → PCIe/GPU BuildKit custody'sine taşınır ve visible ticket `9/10 → 10/10` olur.
- Inventory Decuple all-or-none claim, domain-first world projection, same Unity instance/stable ItemId, live reservation/allocation, exact replay/revision ve stable recovery korunur. Generic transfer/drop/box/stack/cart, installed route ve Assembly bypass'ları fail-closed'dur.
- GarageGraybox r44 keyboard/mouse + gamepad, keyed `0° ↔ 180°` preview ve single-consumer BuildKit/installed-route/Assembly arbitration taşır. Issue #63 PCIe/GPU, #61 ATX24, #62 EPS12V, #59 GPU, #60 PSU ve diğer Assembly state/revision/receipts unchanged kalır.
- Technical commit `25dc39ab02de93a416800acd17f53aacf83dca09`, tree `a736a764d0a52e950a4139002d6febc629df5987`; full EditMode `709/709`, PlayMode `116/116`; Universal Mac report `330073048` bayt, Apple M1/Metal exact r44 smoke ve technical Guard `32921526334` başarılıdır.
- Complete bundle ile collision-free detached-clean Windows x64 IL2CPP/only-D3D11 report `1339592274` bayt, `issue87-hardened-v1` fatal-token `0`; Intel Iris Xe/Direct3D 11.0 feature level 11.1 exact runtime, graceful exit, task deletion ve residue `0` ile geçti.
- ADR-0053/tarihli Evidence ve `issue87` verifier contract'ı teknik kanıtı bağlar. `prerequisite-positioning=teleport-assisted` nedeniyle otomatik smoke gerçek insan kabulü değildir. Source/docs Guard, final canonical `14/14`, immutable local/healthy physical USB, metadata/Guard ve exact-r44 insan oturumu bekler; strict acceptance `23/25`, Issue #87 ve draft PR #88 açık/In Progress kalır.
...
26 Ağustos 2026 itibarıyla üstün gelen güncel durum: Epic #10 altında motherboard/CPU/DIMM/M.2/cooler/GPU/PSU/ATX24/EPS12V/PCIe-GPU 6+2 fiziksel authority dilimleri ile Issue #64 accepted request→immutable ten-line quote/BOM→exact reservation ve Issue #66 BuildOrder/WorkTicket→exact allocation sınırları korunur. Issue #68, #71, #73, #75, #77, #79, #81, #83, #85 ve güncel #87 sırasıyla canonical motherboard, CPU, DDR5, M.2, processor cooler, GPU, PSU, ATX24, EPS12V ve PCIe/GPU cable'ı gerçek pickup/carry/placement ile ayrı capacity-one BuildKit slotlarına taşımış; work-ticket fiziksel staging ilerlemesi `0/10 → 10/10` olmuştur. Issue #87 technical source `25dc39ab02de93a416800acd17f53aacf83dca09` üzerinde fresh `709/709` EditMode, `116/116` PlayMode, Universal Mac/Apple M1 Metal r44, technical Guard `32921526334` ve Windows `issue87-hardened-v1` IL2CPP/Intel Iris Xe D3D11 r44 native kapılarını geçmiştir. Source/docs Guard, final provenance `14/14`, immutable local+sağlıklı fiziksel USB ve exact-r44 insan kabulü henüz bekler; strict acceptance `23/25`, Issue #87 açık/In Progress durumundadır. BuildKit staging tamamlanması Assembly completion veya electrical readiness değildir; component installation/retention, cable routing, electrical power-on/POST/OS/benchmark, Save/Guardian, işlevsel mahalle/ev/araç katmanı ve final art ayrı kalır.
...
> Issue #87 technical source `25dc39a`, tree `a736a76`; fresh EditMode `709/709`, PlayMode `116/116`, Universal Mac report `330073048` bayt, Apple M1/Metal exact r44 native smoke ve technical Guard `32921526334` kapılarını geçti. Canonical PCIe/GPU 8-pin 6+2 cable staged ilk dokuz BuildKit önkoşuluyla exact WorldFloor → ActorHands → PCIe/GPU-specific BuildKit custody'sine taşınır; same-instance/stable serialized identity, live reservation/allocation, `9/10 → 10/10`, keyed 180° rotation, replay/recovery, no-duplicate-loss ve installed-route/Assembly isolation korunur. Collision-free `issue87-hardened-v1` exact-source Windows IL2CPP/Direct3D11 report `1339592274` bayt, geniş fatal-token `0`, Intel Iris Xe exact r44 runtime, task deletion ve residue `0` ile native kapıyı geçti. Source/docs Guard, final canonical `14/14`, immutable local+sağlıklı fiziksel USB ve exact-r44 insan kabulü henüz bekler; strict acceptance `23/25`, Issue #87 ve draft PR #88 açık/In Progress kalır. BuildKit staging `10/10` tamamlanmıştır; sıradaki bounded teknik iş component installation/retention zinciridir ve önceki immutable checkpoint'ler değiştirilmez.
