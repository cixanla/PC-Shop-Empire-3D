...
**Branch:** `feature/issue85-eps12v-power-cable-build-kit-handoff`
...
### 26 Ağustos 2026 üstün gelen güncel durum — Issue #85 EPS12V Power-Cable BuildKit

- Exact reserved `PowerCable` yalnız `ModularEps12v8PinPsuToMotherboard` family ve tam line/product/item/reservation lineage ile seçilir; staged motherboard+CPU+DDR5+M.2+processor-cooler+graphics-card+power-supply+ATX24 sonrasında ayrı stable operation/capacity-one EPS12V slotuyla WorldFloor → ActorHands → EPS12V BuildKit custody'sine taşınır ve visible ticket `8/10 → 9/10` olur.
- Inventory Nonuple all-or-none claim, domain-first world projection, same Unity instance/stable ItemId, live reservation/allocation, exact replay/revision ve stable recovery korunur. Generic transfer/drop/box/stack/cart, installed route ve Assembly bypass'ları fail-closed'dur.
- GarageGraybox r43 keyboard/mouse + gamepad, keyed `0° ↔ 180°` preview ve single-consumer BuildKit/installed-route/Assembly arbitration taşır. Issue #62 EPS12V, #61 ATX24, #63 PCIe/GPU, #60 PSU bay ve diğer Assembly state/revision/receipts unchanged kalır.
- Technical commit `b6a74e932f4744b17388df7c7eb4d88f26d195f4`, tree `bd763ea0c8c6d2f5d256e467c4fca8b762ca4d84`; full EditMode `705/705`, PlayMode `115/115`; Universal Mac report `330018708` bayt ve Apple M1/Metal exact r43 smoke başarılıdır.
- Complete bundle ile detached-clean Windows x64 IL2CPP/only-D3D11 report `1338310618` bayt, `issue85-hardened-v1` fatal-token `0`; Intel Iris Xe/Direct3D 11.0 feature level 11.1 exact runtime, graceful exit, task deletion ve residue `0` ile geçti.
- ADR-0052/tarihli Evidence ve `issue85` verifier contract'ı teknik kanıtı bağlar. `prerequisite-positioning=teleport-assisted` nedeniyle otomatik smoke gerçek insan kabulü değildir. Source/docs Guard, final canonical `14/14`, immutable local/healthy physical USB, metadata/Guard ve exact-r43 insan oturumu bekler; strict acceptance `22/25`, Issue #85 açık/In Progress kalır.
...
26 Ağustos 2026 itibarıyla üstün gelen güncel durum: Epic #10 altında motherboard/CPU/DIMM/M.2/cooler/GPU/PSU/ATX24/EPS12V/PCIe-GPU 6+2 fiziksel authority dilimleri ile Issue #64 accepted request→immutable ten-line quote/BOM→exact reservation ve Issue #66 BuildOrder/WorkTicket→exact allocation sınırları korunur. Issue #68, #71, #73, #75, #77, #79, #81, #83 ve güncel #85 sırasıyla canonical motherboard, CPU, DDR5, M.2, processor cooler, GPU, PSU, ATX24 ve EPS12V cable'ı gerçek pickup/carry/placement ile ayrı capacity-one BuildKit slotlarına taşımış; work-ticket fiziksel ilerlemesi `0/10 → 9/10` olmuştur. Issue #85 technical source `b6a74e932f4744b17388df7c7eb4d88f26d195f4` üzerinde fresh `705/705` EditMode, `115/115` PlayMode, Universal Mac/Apple M1 Metal r43 ve Windows `issue85-hardened-v1` IL2CPP/Intel Iris Xe D3D11 r43 native kapılarını geçmiştir. Source/docs Guard, final provenance `14/14`, immutable local+sağlıklı fiziksel USB ve exact-r43 insan kabulü henüz bekler; strict acceptance `22/25`, Issue #85 açık/In Progress durumundadır. Sonraki teknik bounded bağımlılık PCIe/GPU BuildKit `9/10 → 10/10` adımıdır. Assembly completion, cable routing, electrical power-on/POST/OS/benchmark, Save/Guardian, işlevsel mahalle/ev/araç katmanı ve final art ayrı kalır.
...
> Issue #85 technical source `b6a74e9`, tree `bd763ea`; fresh EditMode `705/705`, PlayMode `115/115`, Universal Mac report `330018708` bayt ve Apple M1/Metal exact r43 native smoke kapılarını geçti. Canonical EPS12V cable staged ilk sekiz BuildKit önkoşuluyla exact WorldFloor → ActorHands → EPS12V-specific BuildKit custody'sine taşınır; same-instance/stable serialized identity, live reservation/allocation, `8/10 → 9/10`, keyed 180° rotation, replay/recovery, no-duplicate-loss ve installed-route/Assembly isolation korunur. Collision-free `issue85-hardened-v1` exact-source Windows IL2CPP/Direct3D11 report `1338310618` bayt, geniş fatal-token `0`, Intel Iris Xe exact r43 runtime, task deletion ve residue `0` ile native kapıyı geçti. Source/docs Guard, final canonical `14/14`, immutable local+sağlıklı fiziksel USB ve exact-r43 insan kabulü henüz bekler; strict acceptance `22/25`, Issue #85 açık/In Progress kalır. Sıradaki bounded teknik iş PCIe/GPU cable BuildKit ve görünür `9/10 → 10/10` ilerlemedir; önceki immutable checkpoint'ler değiştirilmez.
