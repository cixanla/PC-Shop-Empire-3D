...
Issue #139 Quality-Bound Physical Custom-PC Packaging and Dispatch Custody r68 technical head `79ea367af67549592a6ba58acd53afa74e7f25cb`, tree `12dabe0220ffe759750d73cc25e96e2c6774221d` üzerinde Mac-ready durumdadır. Unity-independent `PSE.Fulfillment`, exact current `ReadyForPackaging` quality release'ini tek stable package/seal identity'sine ve append-only custody history'sine bağlar. PackagingWorkbench → ActorHands → WorldFloor/TransportCart/DispatchStaging geçişleri immutable receipt'lerle doğrulanır; history-before-replay, same-instance replay, changed-reuse conflict, revision/overflow/tamper fail-closed ve physical-move/domain-commit rollback korunur.

Mac targeted `4/4 + 13/13 + 4/4 + 2/2 + 2/2`, final full `815/815 EditMode + 195/195 PlayMode`, universal `330891503` bayt/`306` dosya build ve Apple M1/Metal 1280×720 keyboard+mouse+virtual-gamepad quality-release→seal→pickup→cart→pickup→dispatch smoke geçti. Exact result `package=one`, `physical=large-box`, `source-projections=10 hidden-after-seal=true`, `custody-receipts=4`, replay/history/upstream invariants intact'tir. Draft PR #140 açıktır; Issue/Roadmap `In Progress`, parent Epic #10 ve Steam 1.0 Goal açıktır. Bu yalnız bounded physical packaging/dispatch-staging custody dilimidir: courier handoff, customer delivery, settlement, fault/repair, save/load ve warranty ayrı sonraki dilimlerdir. Fiziksel Windows olmadığından clean exact-source x64 IL2CPP/D3D11/Iris Xe gate; USB olmadığından immutable checkpoint/readback ertelenmiştir. UTM fiziksel release kanıtının yerine geçmez. Mac tek authoritative write lane'de sıradaki bounded delivery/courier/settlement zincirini sürdürür.
