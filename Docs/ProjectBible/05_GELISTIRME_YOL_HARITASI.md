...
**Son bütünlük QA güncellemesi:** 11 Ağustos 2026  
...
**Durum:** Devam ediyor; teknik temel, oynanabilir garaj, küçük-kutu pickup/drop/placement/rotation ve güvenli büyük-kutu taşıma tamamlandı; tek-köşe görsel benchmark sırada.<br>
**Öncelik:** P0  
...
**Güncel teknik kanıt:** `PSE.Core` Unity bağımlılığı olmadan derlenir; stable ID, result, zaman, RNG ve event dispatcher sözleşmeleri tamamdır. `PSE.World`/`PSE.Presentation`, GarageGraybox, connected PlayerRig, klavye/fare + gamepad hareket/kamera, görünür eller, stable pickup/drop/recovery, işaretli yüzeyde deterministik küçük-kutu placement/90° rotation ve ayrı büyük-kutu taşıma profili içerir. Büyük kutu iki-el pozu, `0,65×` hareket, sprint kilidi, motion-safe bounded FOV ve gerçek boyutlu fail-closed drop kullanır. Edit Mode 127/127, gerçek device-state Play Mode 10/10, Universal macOS development build ve Apple M4/Metal `rotation=ok` runtime smoke geçer. Sıradaki bounded paket Issue #34 ile tek-köşe okunaklı yarı gerçekçi görsel benchmarktır; final-art dönüşümü, istifleme, taşıma arabası ve Inventory authority ayrı kalır.
