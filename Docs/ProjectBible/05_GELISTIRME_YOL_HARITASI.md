...
**Güncel teknik kanıt:** `PSE.Core`, `PSE.Catalog`, `PSE.Inventory`, `PSE.Orders` ve `PSE.Retail` Unity bağımsız derlenir. Exact manifest atomik Receiving intake üretir; `PSE.Presentation` aynı serialized item'ı görünür teslimat → acceptance/Receiving → idempotent parcel open → ActorHands → RAF A Shelf/WorldFloor → offer publish → customer basket reserve/release zincirine bağlar. Offer integer minor-unit fiyatı stable product+shelf kimliğiyle tutar; basket line exact offer+item+Inventory claim'e bağlıdır. Duplicate talep ve drift failure no-mutation, reserved pickup fail-closed kalır. Edit Mode 220/220, gerçek device-state Play Mode 17/17, Universal macOS ve Apple M4/Metal `basket-reservation=ok release=ok` runtime geçer. Sıradaki paket immutable checkout price snapshot sınırıdır; reservation consume, dinamik ekonomi/satış/full final-art ayrı kalır.
