...
Issue #129 Deterministic UEFI Baseline r63 technical head `86df0bc236e2bf90bfc3fa0482715f06242e6f13` üzerinde Mac-ready durumdadır. Existing power authority exact current POST/power/preflight lineage'ına bağlı ayrı immutable firmware receipt history'si taşır. One-per-POST, same-instance replay, historical preservation, active-clear ve malformed-history sırasında explicit power-off recovery doğrulanır; ikinci authority veya gameplay mutation'ı yoktur.

Mac targeted `3/3 + 1/1 + 5/5 + 10/10`, full `784/784 EditMode + 169/169 PlayMode`, universal `330573681` bayt/`302` dosya build ve Apple M1/Metal 1280×720 keyboard+mouse+virtual-gamepad power-on→POST→UEFI review→save/exit→power-off, replay, presentation, untouched benchmark ve invariants smoke geçti. Draft PR #130 açıktır; Guard `33367768909` PASS, Issue/Roadmap `In Progress`, parent Epic #10 ve Steam 1.0 Goal açıktır. Bu yalnız kurgusal safe-default receipt'tir: gerçek firmware flashing/vendor BIOS, OS/driver ve benchmark ayrı sonraki dilimlerdir. Fiziksel Windows olmadığından clean exact-source x64 IL2CPP/D3D11/Iris Xe gate; USB olmadığından immutable checkpoint/readback ertelenmiştir. UTM fiziksel release kanıtının yerine geçmez. Mac tek authoritative write lane'de bounded ürün zincirini sürdürür.
