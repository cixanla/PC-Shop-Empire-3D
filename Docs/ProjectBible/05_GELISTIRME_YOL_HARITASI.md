...
Issue #131 Deterministic Fictional OS Installation r64 technical head `9e6a2334a3d6d778b97ebb9ee6d43e7cd8dbc31f` üzerinde Mac-ready durumdadır. Separate OS authority exact current UEFI→POST→power→preflight ve exact physical M.2 item/product/secure-operation/full Assembly revision lineage'ına bağlı immutable receipt history'si taşır. Same-instance replay, same-item power-off/reseat persistence, different-item rejection ve malformed-history sırasında explicit power-off recovery doğrulanır; upstream gameplay authority mutation'ı yoktur.

Mac targeted `4/4 + 1/1 + 6/6 + 17/17`, full `788/788 EditMode + 175/175 PlayMode`, universal `330604881` bayt/`302` dosya build ve Apple M1/Metal 1280×720 keyboard+mouse+virtual-gamepad power-on→POST→UEFI→OS review/install→power-off, storage persistence, replay, presentation, untouched benchmark ve invariants smoke geçti. Draft PR #132 açıktır; Guard `33372528502` PASS, Issue/Roadmap `In Progress`, parent Epic #10 ve Steam 1.0 Goal açıktır. Bu yalnız bounded fictional OS receipt'idir: gerçek OS media/disk/reboot, driver ve benchmark ayrı sonraki dilimlerdir. Fiziksel Windows olmadığından clean exact-source x64 IL2CPP/D3D11/Iris Xe gate; USB olmadığından immutable checkpoint/readback ertelenmiştir. UTM fiziksel release kanıtının yerine geçmez. Mac tek authoritative write lane'de bounded driver-preparation zincirini sürdürür.
