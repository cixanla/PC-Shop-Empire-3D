...
**Güncel teknik kanıt:** `PSE.Core`, `PSE.Catalog`, `PSE.Inventory`, `PSE.Orders` ve `PSE.Retail` Unity bağımsız derlenir. Exact manifest atomik Receiving intake üretir; `PSE.Presentation` aynı serialized item'ı görünür teslimat → acceptance/Receiving → idempotent parcel open → ActorHands → RAF A Shelf/WorldFloor → offer publish → customer basket reserve/release → checkout begin zincirine bağlar. Checkout bütün basket satırlarını exact offer+item+Inventory claim ile doğrular, integer price/currency/total'i immutable snapshot eder; daha sonraki offer update'i açık fiyatı değiştirmez. Duplicate/drift failure no-mutation, checkout aktif release/pickup fail-closed kalır. Edit Mode 233/233, gerçek device-state Play Mode 17/17, Universal macOS ve Apple M4/Metal `checkout-snapshot=ok price-frozen=ok` runtime geçer. Sıradaki paket reservation consume + serialized sold/fulfilled commit sınırıdır; ödeme/Economy, Save, AI ve final art ayrı kalır.
