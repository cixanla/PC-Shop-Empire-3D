...
Issue #123 Player-Triggered Power-Test Preflight r60 technical head `3c26ce0d6de80c975b064f2dff68d96fbd4378bc` üzerinde Mac-ready durumdadır. Exact electrical-readiness + `380/500/550 W` budget lineage, ayrı reference-bound attempt authority üzerinden oyuncunun existing Workbench'te normal Interact action'ıyla istediği tek immutable preflight receipt'ine taşındı. Historical exact replay same-instance kalır; current cable/Assembly drift `ContextStale` üretir ve hiçbir gameplay authority state'ini değiştirmez.

Mac targeted `6/6 + 3/3`, full `773/773 EditMode + 161/161 PlayMode`, universal `330507808` bayt build ve Apple M1/Metal keyboard+gamepad/single-consumer/range/focus/LOS/pause/co-edge/receipt/replay/stale/invariant smoke geçti. Draft PR #124 açıktır; Issue/Roadmap `In Progress`, parent Epic #10 ve Steam 1.0 Goal açıktır. Gerçek power-on, connector fault/polarity fiziği, POST/BIOS/OS/benchmark ayrı sonraki dilimlerdir. Fiziksel Windows olmadığından clean exact-commit x64 IL2CPP/D3D11/Iris Xe gate; USB olmadığından immutable checkpoint/readback ertelenmiştir. UTM fiziksel release kanıtının yerine geçmez. Mac tek authoritative write lane'de bounded ürün zincirini sürdürür.
