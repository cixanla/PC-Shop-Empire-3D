...
Issue #133 Deterministic Fictional Driver Installation r65 technical head `b144a3ef1a0ac5fcbd9704c850426baa9a727044` üzerinde Mac-ready durumdadır. Separate driver authority exact current installed OS/storage ve current firmware→POST→power→preflight owner chain'ine bağlı immutable receipt history'si taşır. Completion exact component/product/retain/cable lineage'ını fail-closed ister; same-instance replay ve no-upstream-mutation korunur. Completion sonrasında same current OS/storage üzerindeki Installed sonucu non-storage hardware değişiminden etkilenmez.

Mac targeted `5/5 + 6/6 + 1/1 + 23/23`, full `793/793 EditMode + 181/181 PlayMode`, universal `330641904` bayt/`302` dosya build ve Apple M1/Metal 1280×720 keyboard+mouse+virtual-gamepad power-on→POST→UEFI→OS→driver review/install→power-off, OS/storage persistence, replay, `Waiting/Reviewing/Installed/Rejected` presentation, untouched benchmark ve invariants smoke geçti. Draft PR #134 açıktır; Guard `33378476265` PASS, Issue/Roadmap `In Progress`, parent Epic #10 ve Steam 1.0 Goal açıktır. Bu yalnız bounded fictional driver receipt'idir: gerçek vendor driver/update/reboot ile benchmark/stress/thermals/quality ayrı sonraki dilimlerdir. Fiziksel Windows olmadığından clean exact-source x64 IL2CPP/D3D11/Iris Xe gate; USB olmadığından immutable checkpoint/readback ertelenmiştir. UTM fiziksel release kanıtının yerine geçmez. Mac tek authoritative write lane'de bounded benchmark-preparation zincirini sürdürür.
