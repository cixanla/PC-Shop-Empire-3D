...
**Güncel teknik kanıt:** `PSE.Core`, `PSE.Catalog`, `PSE.Inventory` ve `PSE.Orders` Unity bağımsız derlenir. Exact manifest atomik Receiving intake üretir; `PSE.Presentation` aynı serialized item'ı görünür teslimat → acceptance/Receiving → idempotent parcel open → ActorHands → RAF A Shelf/WorldFloor zincirine bağlar. Opening quantity/domain revision değiştirmez; invalid state/identity/location no-reveal kalır. Edit Mode 192/192, gerçek device-state Play Mode 17/17, Universal macOS ve Apple M4/Metal `parcel-open=ok` runtime geçer. Sıradaki paket authoritative shelf offer/fiyat etiketi ve RAF A label projection'ıdır; dinamik ekonomi/satış/full final-art ayrı kalır.
