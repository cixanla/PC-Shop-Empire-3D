...
**Güncel teknik kanıt:** `PSE.Core`, `PSE.Catalog`, `PSE.Inventory`, `PSE.Orders` ve `PSE.Retail` Unity bağımsız derlenir. Exact manifest atomik Receiving intake üretir; `PSE.Presentation` aynı serialized item'ı görünür teslimat → acceptance/Receiving → idempotent parcel open → ActorHands → RAF A Shelf/WorldFloor → offer publish zincirine bağlar. Offer integer minor-unit fiyatı stable product+shelf kimliğiyle tutar; publish quantity veya Inventory/Orders revision değiştirmez. Edit Mode 207/207, gerçek device-state Play Mode 17/17, Universal macOS ve Apple M4/Metal `shelf-offer=ok price-minor=54999 currency=EUR` runtime geçer. Sıradaki paket customer basket/serialized reservation sınırıdır; checkout snapshot, dinamik ekonomi/satış/full final-art ayrı kalır.
