...
**Güncel teknik kanıt:** `PSE.Core`, `PSE.Catalog`, `PSE.Inventory`, `PSE.Orders` ve `PSE.Retail` Unity bağımsız derlenir. Exact manifest atomik Receiving intake üretir; `PSE.Presentation` aynı serialized item'ı görünür teslimat → acceptance/Receiving → idempotent parcel open → ActorHands → RAF A Shelf/WorldFloor → offer publish → customer basket reserve/release → checkout begin → fulfillment zincirine bağlar. Checkout bütün basket satırlarını exact offer+item+Inventory claim ile doğrular, integer price/currency/total'i immutable snapshot eder; daha sonraki offer update'i açık fiyatı değiştirmez. Completion bütün reservation setini preflight edip Inventory, Basket ve Checkout revision'larını birer kez ilerletir; serialized item/projection kaldırılır, exact tekrar idempotent ve drift failure no-mutation kalır. Edit Mode 242/242, gerçek device-state Play Mode 17/17, Universal macOS ve Apple M4/Metal `sale-completion=ok stock-consumed=ok completed-quantity=0` runtime geçer. Ödeme/Economy, Save, müşteri AI ve final art ayrı kalır; sıradaki bounded alan Issue #9 müşteri intent/state davranışıdır.
