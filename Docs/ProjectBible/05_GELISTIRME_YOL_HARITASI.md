...
**Güncel teknik kanıt:** `PSE.Core`, `PSE.Catalog`, `PSE.Inventory` ve `PSE.Orders` Unity bağımsız derlenir. Purchase order exact manifesti yalnız arrival sonrası, bütün identity/tracking/capacity preflight'ı geçince receiving container'a tek revision'da girer; failure iki authority'yi değiştirmez. `PSE.Presentation` GarageGraybox'ta aynı serialized item'ı görünür teslimat → Receiving → ActorHands → RAF A Shelf/WorldFloor zincirine domain-first transfer, rollback ve recovery ile bağlar. Edit Mode 188/188, gerçek device-state Play Mode 17/17, Universal macOS ve Apple M4/Metal `stock-flow=ok` runtime geçer. Sıradaki paket fiziksel koli açma/manifest projection'ıdır; fiyat/satış ve full final-art ayrı kalır.
