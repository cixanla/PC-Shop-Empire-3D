...
| D-170 | Tamamlanan geliştirme paketleri USB'ye her dosya kaydında değil, doğrulanmış milestone snapshotı olarak yazılır. | Kullanıcı onayı — uygulandı 15 Ağustos 2026 | Güncel `2026-08-15_STAGE_B_ORDER_RECEIVING` snapshotı 449 tracked source + 4 test evidence + source kaydı taşır; 454 manifest satırı `07480d15…485cff` SHA-256, tam readback ve source checksum kapılarından mismatch `0` ile geçti. `.git`, cache, build uygulaması, credential ve AppleDouble dışarıda kalır. |
