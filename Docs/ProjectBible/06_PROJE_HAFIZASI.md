...
| D-170 | Tamamlanan geliştirme paketleri USB'ye her dosya kaydında değil, doğrulanmış milestone snapshotı olarak yazılır. | Kullanıcı onayı — uygulandı 15 Ağustos 2026 | Güncel `2026-08-15_STAGE_B_AUTHORITATIVE_STOCK_FLOW` snapshotı source `f20fd17`, 467 tracked source + 4 test/build/runtime evidence + source kaydı taşır; 472 manifest satırı `5521f869…22a3` SHA-256, tam readback/source checksum kapılarından mismatch `0` ile geçti. `.git`, cache, büyük build uygulaması, credential ve AppleDouble dışarıda kalır. |
