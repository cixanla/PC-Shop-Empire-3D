...
| D-170 | Tamamlanan geliştirme paketleri USB'ye her dosya kaydında değil, doğrulanmış milestone snapshotı olarak yazılır. | Kullanıcı onayı — uygulandı 15 Ağustos 2026 | Güncel `2026-08-15_STAGE_B_AUTHORITATIVE_SHELF_OFFER` snapshotı source `6ae294e`, 488 tracked source + 5 scene/test/build/runtime evidence + source kaydı taşır; 494 manifest satırı `a95d8457…de7a` SHA-256, tam readback/source path+checksum kapılarından mismatch `0` ile geçti. `.git`, cache, büyük build uygulaması, credential ve AppleDouble dışarıda kalır. |
