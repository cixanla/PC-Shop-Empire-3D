...
| D-170 | Tamamlanan geliştirme paketleri USB'ye her dosya kaydında değil, doğrulanmış milestone snapshotı olarak yazılır. | Kullanıcı onayı — uygulandı 15 Ağustos 2026 | Güncel `2026-08-15_STAGE_B_DELIVERY_PARCEL_UNPACKING` snapshotı source `756547f`, 471 tracked source + 5 scene/test/build/runtime evidence + source kaydı taşır; 477 manifest satırı `37f95b3c…58ac` SHA-256, tam readback/source checksum kapılarından mismatch `0` ile geçti. `.git`, cache, büyük build uygulaması, credential ve AppleDouble dışarıda kalır. |
...
Paket ve Stage A kapsamı 11 Ağustos 2026'da onaylandı ve uygulandı. Stage B Core, oynanabilir GarageGraybox fiziksel akışı, görsel benchmark, Catalog + Inventory authority, atomik purchase-order receiving, transactional dünya/stok projection'ı ve idempotent parcel reveal ayrı testli paketlerde kapatıldı. Güncel baseline Edit Mode 192/192 ve Play Mode 17/17'dir; sıradaki bounded iş authoritative shelf offer/fiyat etiketi ve RAF A label projection'ıdır.
