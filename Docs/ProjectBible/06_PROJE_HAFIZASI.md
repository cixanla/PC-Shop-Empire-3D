...
| D-170 | Tamamlanan geliştirme paketleri USB'ye her dosya kaydında değil, doğrulanmış milestone snapshotı olarak yazılır. | Kullanıcı onayı — uygulandı 15 Ağustos 2026 | Güncel `2026-08-15_STAGE_B_CATALOG_INVENTORY` snapshotı 428 tracked source + 4 test evidence + source kaydı taşır; 433 manifest satırı `f481ddfa…49dc9` SHA-256, tam readback ve source checksum kapılarından mismatch `0` ile geçti. `.git`, cache, build uygulaması, credential ve AppleDouble dışarıda kalır. |
...
Paket ve Stage A kapsamı 11 Ağustos 2026'da onaylandı ve uygulandı. Stage B Core, oynanabilir GarageGraybox fiziksel akışı, görsel benchmark, Catalog + Inventory authority ve atomik purchase-order receiving ayrı testli paketlerde kapatıldı. Güncel baseline Edit Mode 184/184 ve Play Mode 14/14'tür; sıradaki bounded iş visible delivery/receiving/raf adaptörüdür.
