...
| D-170 | Tamamlanan geliştirme paketleri USB'ye her dosya kaydında değil, doğrulanmış milestone snapshotı olarak yazılır. | Kullanıcı onayı — uygulandı 15 Ağustos 2026 | Güncel `2026-08-15_STAGE_B_CUSTOMER_BASKET_RESERVATION` snapshotı source `109237a`, 498 tracked source + 4 test/build/runtime evidence + source kaydı taşır; 503 manifest satırı `ff868e4c…20d7` SHA-256, tam readback/source path+checksum kapılarından mismatch `0` ile geçti. `.git`, cache, büyük build uygulaması, credential ve AppleDouble dışarıda kalır. |
