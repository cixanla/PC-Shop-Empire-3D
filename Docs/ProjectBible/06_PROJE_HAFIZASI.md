...
**Son güncelleme:** 24 Ağustos 2026<br>
...
| D-170 | Tamamlanan geliştirme paketleri USB'ye her dosya kaydında değil, doğrulanmış milestone snapshotı olarak yazılır. | Kullanıcı onayı — uygulandı 15 Ağustos 2026 | Güncel `2026-08-15_STAGE_B_ATOMIC_CHECKOUT_FULFILLMENT` snapshotı source `80eea8f`, 510 tracked source + 4 test/build/runtime evidence + source kaydı ve 9.373.684 bayt taşır; 515 manifest satırı `ce72122a…db50b` SHA-256, tam readback/source path+Git-blob kapılarından mismatch `0` ile geçti. `.git`, cache, büyük build uygulaması, credential ve AppleDouble dışarıda kalır. |
...
- Yirmi üçüncü Stage B paketi `b37b056271fac317e99ec47df0833b8ef219cf83` commit'iyle Unity bağımsız `PSE.Actors`, stable customer/intent/visit, bounded exact receipt ledger, iki denemeli route fallback ve patience/exit timeout ekledi. Garage runtime NavMesh offer→RAF A→checkout→fulfilled exit akışını gösterir; pause clock/ajanı dondurur ve Inventory/Retail/Orders authority izolasyonu korunur. Edit Mode 255/255, Play Mode 18/18, Universal macOS ve Apple M4/Metal normal+leakdiag `runtime-route=ok fulfilled=ok authority-isolated=ok` geçti. Sırada tek offer açıklanabilir satın-al/ayrıl kararı vardır.
- Issue #46 kapanışı source/docs `d163328`, Repository Guard `31875627062` ve `2026-08-15_STAGE_B_DETERMINISTIC_CUSTOMER_VISIT` USB milestone'ıyla sabitlendi: 535 tracked source + 5 evidence + source kaydı, 541 satırlı `c82fc76d…cfd` manifest, 535/535 Git-blob eşliği; forbidden/credential/AppleDouble `0`.
- Yirmi dördüncü Stage B paketi `f97ded34f00e0d0637fbf9b41c0c0d33a7969b8e` commit'iyle pure `CustomerOfferDecisionEvaluator`, immutable/equatable provenance, deterministic `Buy/Leave` reasonları ve visible Garage karar metnini ekledi. Karar okuması hiçbir authority'yi veya lifecycle'ı değiştirmez; gerçek keyboard/gamepad izolasyonu testlidir. Edit Mode 267/267, Play Mode 18/18, Universal macOS ve Apple M4/Metal `garage-offer-decision-r16-v1 offer-decision=ok authority-isolated=ok` geçti. Source/docs `8832c13`, Guard `31877488552`, doğrulanmış `EXPLAINABLE_SINGLE_OFFER_CUSTOMER_DECISION` USB milestone'ı ve Issue #47 Done/close ile kapandı.
- Yirmi beşinci Stage B paketi `6951869c4a9f33662f322c02348fa4282b9cdbb6` commit'iyle explicit Actors↔Retail binding, stale-safe current `Buy`, exact action-owned reservation ve `Browsing → NavigatingToCheckout` action ledger'ını ekledi. Edit Mode 287/287, Play Mode 19/19, Universal macOS ve Apple M4/Metal `garage-buy-action-r17-v1 buy-action=ok stale-blocked=ok authority-isolated=ok` geçti. Source/docs `aa61700`, Guard `31880730059`, doğrulanmış `STALE_SAFE_BUY_ACTION_AND_CHECKOUT_NAVIGATION` USB milestone'ı ve Issue #48 Done/close ile kapandı.
- Yirmi altıncı Stage B paketi `67d858aff773610cff6d6c221c792cd793f27a1b` commit'iyle ortak kind-discriminated action ledger'ına stale-safe current `Leave`, internal Actors prepared planı, `Browsing → Exiting`, stable `OfferDeclined` ve Browse→Exit NavMesh kontratını ekledi. Commerce authority izolasyonu, cross-kind replay engeli ve historical receipt testlidir. Edit Mode 298/298, Play Mode 22/22, Universal macOS ve Apple M4/Metal `garage-leave-action-r18-v1 leave-action=ok stale-leave-blocked=ok authority-isolated=ok` geçti. Source/docs `868885a`, Guard `31882508496`, doğrulanmış `STALE_SAFE_LEAVE_ACTION_AND_OFFER_DECLINED_EXIT` USB milestone'ı ve Issue #49 Done/close ile kapandı. Sırada atomik payment/ilk Economy settlement sınırı vardır.
...
- Source/docs `01c2b5a49f11b27b52af9e299d4d2e48cef3c962`, USB metadata `17af550856e8bca288ed5c17924bc82586c76c27`; [Repository Guard 31919985055](https://github.com/cixanla/PC-Shop-Empire-3D/actions/runs/31919985055), [31920258176](https://github.com/cixanla/PC-Shop-Empire-3D/actions/runs/31920258176) ve [31920923402](https://github.com/cixanla/PC-Shop-Empire-3D/actions/runs/31920923402) başarılıdır.
...
- Kapanış: source/docs `2e848e3`, Guard `32591206866` + `32591381804`; USB-erteleme kapanış metadatası `fce6bfa`, Guard `32593034745`; acceptance `19/19`, Issue/Roadmap `Done`. Fiziksel USB kullanıcı talimatıyla ertelendi; 717/717 doğrulanmış `f7b2b9bafee9529d95431bbc90914ba51ab24e01de9a0d5d77a53f26cb5626a5` yerel staging hazırdır ve geliştirmeyi bloklamaz.
- Sınır: ayrı paste/reapplication, liquid cooling, GPU/PSU/cabling, POST/BIOS/OS, tam benchmark, final art ve Windows/Steam ayrı issue'lardır.

## Güncel uygulama kaydı — Issue #59 deterministic single PCIe x16 graphics-card seating ve rear-bracket retention

- Epic #10'un yedinci bounded child'ı Issue #59'dur. Feature `1b29ad29d6e4d8cc1ce09b0989a038a72297585c`, tree `e7b3d5d4ad13bf08f822570966660ab6c48e6a55`; canonical Northstar A60 ProductId'sini paylaşan fakat retail itemdan ayrı exact serialized assembly GPU item'ı ve immutable PCIe x16 slot/latch/rear-bracket/fastener topology ekledi. Shadow SKU yoktur.
- Karar: Workbench + ProcessorSocket + MemorySlot + StorageSlot + ProcessorCoolerSlot + capacity-1 GraphicsCardSlot all-or-none claim edilir. Seat/remove exact custody'yi değiştirir; retain/unretain Inventory revision'ını değiştirmez; partial/ghost claim yasaktır.
- Karar: yalnız primary keyed orientation ve exact PCIe x16 interface geçerlidir. Secured host, insertion support, chassis clearance, cooler clearance, range/focus/LOS ve obstruction/tie/saturation kapıları fail-closed'dur.
- Karar: seat/retain/unretain/remove immutable receipts ve exact delayed replay taşır. Duplicate seat, retained remove ve GPU installed iken motherboard detach no-mutation olarak engellenir; retained GPU yalnız kendi benchmark-readiness nedenini giderir ve sonuç `BuildIncomplete` kalır.
- GarageGraybox r28 dual-fan shroud, PCB, PCIe contacts, rear bracket, slot latch ve görünür screw taşır. Gerçek keyboard/mouse ve gamepad dynamic compact HUD/co-edge/pause akışları testlidir; presentation authority değildir, generic placement/stack/cart bypass'ı yoktur ve recovery aynı Unity instance'ı korur.
- Kanıt: EditMode `548/548`, gerçek Input System PlayMode `43/43`, Universal macOS `328781520` bayt ve aktif Apple Silicon/Metal 1280×720 exact `GARAGE_GPU_RUNTIME_SMOKE ... clearance=ok slot-latch=ok rear-bracket=ok ... host-detach-gate=ok replay=ok authority-isolated=ok identity=stable recovery=ok` başarılıdır. Runtime makineyi Apple M1 olarak tanımlar.
- Kapanış: feature `1b29ad2`, source/docs `a5bbca4`; Guard `32599710154` + `32600012769`; acceptance `20/20`, Issue `CLOSED/COMPLETED`, Roadmap `Done`. Parent Epic #10 açık/In Progress kalır. Fiziksel USB kullanıcı haberine kadar ertelendi; Issue #59 için henüz milestone/readback iddiası yoktur.
- Sınır: PSU, PCIe power cabling, alternate GPU dimensions/slots, riser/multi-GPU, POST/BIOS/OS, tam benchmark, final art ve Windows/Steam ayrı issue'lardır.

## Güncel uygulama kaydı — Issue #60 deterministic single ATX PS/2 PSU seating ve four-screw rear retention

- Epic #10'un sekizinci bounded child'ı Issue #60'tır. Feature `f998d7d1c400c9328afa226f0727e6591c02d4e2`, tree `78d62c46354cda45422ca947df10ba9d6823b7c9`; authored-clearance fix `b6c3ff8e95a4da75161b51cdbcbab87cb529f076`, tree `a15865346f52b6b39d84cec49c70babbc6550b89`; tek canonical serialized ATX PS/2 PSU item'ı ve immutable chassis-owned bay/rear-mount/four-fastener topology ekledi. Shadow SKU ve ikinci authority yoktur.
- Karar: Workbench + ProcessorSocket + MemorySlot + StorageSlot + ProcessorCoolerSlot + GraphicsCardSlot + capacity-1 PowerSupplyBay all-or-none claim edilir. Seat/remove exact custody'yi değiştirir; retain/unretain Inventory revision'ını değiştirmez; partial/ghost claim yasaktır.
- Karar: iki keyed 180° fan orientation, exact ATX interface, filtered-floor support ve rear-plane zorunludur. Production authored chassis blocker listesi yalnız `ChassisBack`, `ChassisLeftRail`, `ChassisRightRail`, `MotherboardTray` içerir; support yüzeyleri dışlanır, cabling ayrı olduğu için cable listesi boştur.
- Karar: seat/retain/unretain/remove immutable receipts, exact receipt-history fold ve immediate/delayed replay taşır. Duplicate seat, retained remove, stale/conflict/full-hands/occupied işlemler no-mutation kalır; PSU chassis-owned olduğundan diğer bileşen montaj sırasını zorlamaz. Retained PSU yalnız kendi readiness nedenini giderir ve sonuç `BuildIncomplete` kalır.
- GarageGraybox r29 housing, fan/grille, filtered floor intake, AC inlet, rocker switch, disconnected modular sockets, rear plate ve dört görünür screw taşır. Gerçek keyboard/mouse ve gamepad dynamic compact HUD/co-edge/pause akışları testlidir; presentation authority değildir, generic placement/stack/cart bypass'ı yoktur ve recovery aynı Unity instance'ı korur.
- Kanıt: EditMode `577/577`, gerçek Input System PlayMode `47/47`, Universal macOS `328937592` bayt ve aktif Apple Silicon/Metal 1280×720 exact `GARAGE_PSU_RUNTIME_SMOKE ... clearance=ok rear-mount=ok four-screw=ok ... alternate-order=ok replay=ok authority-isolated=ok identity=stable recovery=ok` başarılıdır. Runtime makineyi Apple M1 olarak tanımlar.
- Kapanış: feature `f998d7d`, authored-clearance fix `b6c3ff8`, source/docs `4939a04`; Guard `32606958882` + `32607437408` + `32607886160`; acceptance `20/20`, Issue `CLOSED/COMPLETED`, Roadmap `Done`. Parent Epic #10 açık/In Progress kalır. Exact local staging 775/775 readback, 770/770 Git-blob, 14.729.691 bayt ve `705784c6…53d4` manifestiyle hazırdır; macOS harici fiziksel USB göstermediği için yanlış volume'a yazılmaz ve fiziksel readback iddiası kurulmaz.
- Sınır: ATX/EPS/PCIe/SATA kablolama, electrical power-on, wattage/headroom, POST/BIOS/OS, completed benchmark, alternate PSU form factors, final art ve Windows/Steam ayrı issue'lardır.

## Güncel uygulama kaydı — Issue #61 deterministic single ATX24 split-PSU cable routing

- Epic #10'un dokuzuncu bounded child'ı Issue #61'dir. Feature `1fc29f13171925c2445eaa7334158e0f058e76a5`, tree `d265332f1d6655639e55db31f9b5a11e3d177f49`; tek canonical serialized ATX24 power cable, typed PSU 18-pin + PSU 10-pin + motherboard 24-pin endpoints ve üç stable ordered route waypoint ekledi. Connector child'lar ayrı Catalog/Inventory/item authority değildir.
- Karar: Workbench + ProcessorSocket + MemorySlot + StorageSlot + ProcessorCoolerSlot + GraphicsCardSlot + PowerSupplyBay + capacity-one CableRoute all-or-none claim edilir. Route/unroute exact custody'yi Hands↔CableRoute taşır; immutable receipt/history fold ve immediate/delayed replay partial/ghost claim olmadan çalışır.
- Karar: retained PSU, secured motherboard, exact endpoint key/orientation, range/focus/LOS ve obstruction-free authored beş segment zorunludur. Duplicate route, wrong key, stale/conflict/saturation, generic placement/stack/cart/raw transfer ve routed durumdaki dependent PSU/motherboard detach no-mutation fail-closed'dur.
- GarageGraybox r30 tek kinematic item root, üç visible connector/latch/key child ve jointsiz LineRenderer route taşır. Dedicated routed connector focus deliberate unroute'a izin verir; presentation authority değildir ve same-instance stable ItemId/recovery korunur. WorldFloor capacity tam dokuzdur.
- Kanıt: EditMode `589/589`, gerçek Input System PlayMode `49/49`, Universal macOS `329082160` bayt ve aktif Apple M1/Metal 1280×720 exact `GARAGE_POWER_CABLE_RUNTIME_SMOKE ... route-waypoints=ok route-clearance=ok ... dependent-detach-blocked=ok replay=ok authority-isolated=ok identity=stable recovery=ok` başarılıdır.
- Kapanış: feature `1fc29f1`, source/docs `52795b6`, USB metadata `f9a5da8`; Guard `32613813494` + `32614187494` + `32632615041`; iki tam 801/801 USB readback, acceptance `20/20`, Issue `CLOSED/COMPLETED`, Roadmap `Done`. Parent Epic #10 açık/In Progress kalır.
- Sınır: EPS/CPU, PCIe/GPU, SATA/Molex/fan/front-panel/data/RGB cabling, electrical power-on, wattage/headroom, POST/BIOS/OS, completed benchmark, free-rope physics, final art ve Windows/Steam ayrı issue'lardır.

## Güncel uygulama kaydı — Issue #62 deterministic single EPS12V/CPU power cable routing

- Epic #10'un onuncu bounded child'ı Issue #62'dir. Feature `15d83aeba0d71238a31bf7b7db5fab3dbd9b5951`, tree `c14524fecee561eff3a144bd15e67be5a48f8335`; tek canonical serialized EPS12V/CPU cable, iki typed/keyed 8-pin endpoint ve üç stable ordered waypoint ekledi. Connector/latch child'lar ayrı Catalog/Inventory/item authority değildir.
- Karar: mevcut sekiz managed container + capacity-one `CpuPowerCableRoute` tek dokuz-container all-or-none claim'dir. Route/unroute exact custody'yi Hands↔CpuPowerCableRoute taşır; immutable receipts, immediate/delayed replay ve history fold item/product/topology/endpoint/waypoint/host/source lineage'ını doğrular.
- Karar: retained PSU, secured motherboard, retained CPU, exact key/orientation, range/focus/LOS ve obstruction-free authored route zorunludur. Wrong key/endpoint, duplicate, stale/conflict/full-hands, NonAlloc saturation, generic placement/stack/cart/raw transfer ve routed durumdaki PSU/motherboard/CPU dependent removal no-mutation fail-closed'dur. ATX24 product/item/container/revision/receipt identities ayrıdır.
- GarageGraybox r31 tek kinematic item root, iki visible connector/latch/key, loose braided presentation ve üç-waypoint jointsiz preview/committed route taşır. Aynı Unity component instance ve stable ItemId gerçek keyboard/mouse veya gamepad pickup→route→unroute→recovery boyunca korunur.
- Kanıt: EditMode `610/610`, gerçek Input System PlayMode `51/51`, Universal macOS `329206153` bayt ve aktif Apple M1/Metal 1280×720 exact `GARAGE_EPS12V_POWER_CABLE_RUNTIME_SMOKE ... cpu-retained-gate=ok ... dependent-detach-blocked=ok replay=ok authority-isolated=ok identity=stable recovery=ok` başarılıdır. Feature Guard `32642211422` geçti.
- Source/docs `cff75f8`, tree `aa5acd7`; Guard `32642638437` başarılıdır. Yerel final staging iki tam 832/832 payload, 826/826 exact Git source ve 5/5 evidence readback'ini geçti; payload 15.757.786 bayt, manifest `afa89feb…6a73`, bütün fark sayaçları `0`dır.
- Kapanış: doğru fiziksel USB milestone'u atomik final adlandırmasıyla sabitlendi; iki tam 832/832 payload, 826/826 exact Git source ve 5/5 evidence readback sıfır fark/AppleDouble ile geçti. USB metadata `2db7cf9`, Guard `32672086464`; acceptance `21/21`, Issue #62 `CLOSED/COMPLETED`, Roadmap `Done`, parent Epic #10 açık/In Progress durumundadır.
- Sınır: electrical power-on, wattage/headroom, POST/BIOS/OS, completed benchmark, PCIe/GPU, SATA/Molex/fan/front-panel/data/RGB cabling, free-rope physics, final art ve Windows/Steam ayrı issue'lardır.

## Güncel uygulama kaydı — Issue #63 deterministic single PCIe/GPU 8-pin 6+2 power cable routing

- Epic #10'un on birinci bounded child'ı Issue #63'tür. Feature `ea1e51f862d4094936c03bccf9fbfaee7bb7d12b`, tree `ecc32279a8e17e8179114a9b6cfcfe4737827601`; tek canonical serialized PCIe/GPU cable, iki distinct typed/keyed endpoint ve üç stable ordered waypoint ekledi. Connector/latch/6+2 child'lar ayrı Catalog/Inventory/item authority değildir.
- Karar: mevcut dokuz managed container + capacity-one `GpuPowerCableRoute` tek on-container all-or-none claim'dir. Route/unroute exact custody'yi Hands↔GpuPowerCableRoute taşır; immutable receipts, immediate/delayed replay ve history fold item/product/topology/endpoint/waypoint/host/source lineage'ını doğrular.
- Karar: retained PSU, secured motherboard, retained GPU, exact key/orientation, range/focus/LOS ve obstruction-free authored route zorunludur. Wrong key/endpoint, duplicate, stale/conflict/full-hands, NonAlloc saturation, generic placement/stack/cart/raw transfer ve routed durumdaki PSU/motherboard/GPU dependent removal no-mutation fail-closed'dur. ATX24/EPS12V/PCIe product/item/container/revision/receipt identities ayrıdır.
- GarageGraybox r32 tek kinematic item root, loose braided presentation ve üç-waypoint jointsiz preview/committed route taşır. GPU-side anchor moving canonical GPU item'a bağlıdır; aynı Unity component instance ve stable ItemId keyboard/mouse veya gamepad pickup→route→unroute→world-drop/recovery boyunca korunur.
- Explicit visual fix `d655f1a5aab0c882cf40702472ec1b8ad44747ad`, tree `c3fff116317db7e3388e0faf04e38a7ffaa7ce77`; PSU ucunu monolitik 8-pin, GPU ucunu separate 6-pin + 2-pin housing, 6-pin keyed latch, 2-pin retention clip ve `6`/`2` labels olarak üretir. Presentation child'lar collider/joint/raycast authority değildir; bağımsız yeniden denetim kalan P0/P1 bulmadı.
- Kanıt: final committed-scene EditMode `626/626`, gerçek Input System PlayMode `53/53`, Universal macOS `329334656` bayt ve aktif Apple M1/Metal 1280×720 exact `GARAGE_PCIE_GPU_POWER_CABLE_RUNTIME_SMOKE ... gpu-retained-gate=ok ... dependent-detach-blocked=ok replay=ok authority-isolated=ok identity=stable recovery=ok` başarılıdır. Scene `2854602` bayt ve SHA-256 `7fc63ba4…f338`dir.
- Repository: feature Guard `32676069923`, repeatable Windows-gate Guard `32676154473`, explicit 6+2 visual-fix Guard `32677267023` ve source/docs `d597941a20afd0491547513abbc68e0b9d890aab` için Guard `32677495639` başarılıdır.
- Windows teknik kapısı: clean/exact `d597941` kaynağı gerçek Windows x64 IL2CPP Development/StrictMode build'ini `1320679269` report baytıyla tamamladı. Aktif console oturumundaki player Intel Iris Xe / Direct3D 11.0 feature level 11.1 üzerinde canonical r32 readiness ve exact `GARAGE_PCIE_GPU_POWER_CABLE_RUNTIME_SMOKE` marker'ını verdi. Build logu `1554549` bayt / SHA-256 `459e95bb43ab79a1004e13e71b74c8500f484c9cd33e1f698deb7f277f844799`; runtime logu `4765` bayt / `853dd5bd75b63d8938dcd6f9b664e979b43aeafa1409b3678dad143d931b3f9e`dir.
- Dış checkpoint: kullanıcı USB'nin bağlı olmadığını bildirmiştir; USB sorgulanmaz veya yazılmaz. En son doğrulanmış fiziksel zincir Issue #62'dir. Issue #63 için collision-free staging, iki tam fiziksel readback, final metadata/Guard ve Issue/Roadmap kapanışı hâlâ bekler.
- Sınır: electrical power-on, current/voltage, PSU wattage/rail/transient/headroom, POST/BIOS/OS, completed benchmark, SATA/Molex/fan/front-panel/data/RGB cabling, free-rope physics, final art/audio/VFX/UI, Save/Guardian ve Steam ayrı issue/gate'lerdir. #63 kapandıktan sonraki bounded aday accepted custom-PC request → immutable quote/BOM → exact serialized Inventory reservation'dır.

## Güncel uygulama kaydı — Issue #64 accepted custom-PC request, immutable quote/BOM and exact serialized reservation

- Epic #10'un on ikinci bounded child'ı Issue #64'tür. Feature `c7d38845ffccb5ae6e5365e580c238d70f8dac95`, tree `615c9c4398f6a0be16c3a693dd812aa3f5541291`; owned customer consultation'ından stable typed graphics-first request ve immutable on-satırlı quote/BOM üretir.
- Exact BOM rolleri motherboard, CPU, DIMM, M.2 SSD, cooler, GPU, PSU, ATX24, EPS12V ve PCIe/GPU 6+2'dir. Her line exact ProductId/ItemInstanceId/ReservationId, integer minor-unit price ve tek currency taşır; complete-role set, duplicate, metadata, socket compatibility ve budget publication öncesi doğrulanır.
- Inventory tam serialized seti tek managed operation, tek claim ve tek revision ile atomik reserve eder. Claim-keyed ve operation-keyed indexler aynı `ManagedSerializedReservationSetRegistration`ı tutar; operation/claim/access/revision/payload drift'i, eleventh item, external claim adoption ve direct release/consume no-mutation fail-closed'dur.
- Exact replay revision artırmaz. Inventory commit ile quote publication arasındaki kesinti yalnız matching owned operation/access üzerinden recover edilir; ikinci reservation veya revision üretilmez. Retail, customer, basket/checkout/economy ve Assembly authority'leri izoledir.
- GarageGraybox r33 keyboard/mouse ve real Input System gamepad ile consultation→request accepted→quote/BOM ready→10 exact reservations akışını görünür kılar. Range/focus/LOS/pause/co-edge/single-consumer, accepted deadline ve pause-toggle no-lurch testlidir.
- Kanıt: Mac ve Windows EditMode `647/647`, PlayMode `59/59`; Universal Mac `329396456` bayt, Apple M1/Metal exact r33 smoke; Windows x64 IL2CPP `1326137709` report baytı, Intel Iris Xe/Direct3D 11.0 exact r33 smoke. Kısa görünür Mac turu hareket ve pause/resume no-lurch gözlemini ekledi.
- Repository: feature Guard `32698054990` başarılı, draft PR #65 açıktır. USB şu anda bağlı değildir; fiziksel iki-pass readback ve final post-checkpoint Guard tamamlanmadan Issue/Roadmap açık/In Progress kalır.
- Sınır: reservation consume/release into build order, fiziksel assembly completion, electrical power-on, POST/BIOS/OS, benchmark/QA, packaging/delivery, final settlement, Save/Guardian, final art/audio/VFX/UI ve Steam ayrı dependent paketlerdir.

## Güncel uygulama kaydı — Issue #66 immutable custom-PC work order and physical work ticket handoff

- Epic #10'un on üçüncü bounded child'ı Issue #66'dır. Core feature `f9545605baff423f05615e7326902e24dc82aeeb`, current technical head `f8afd62326c74aff23fa10bb33ef79ecb9a656b6`, tree `69ea366cc49e99b653f5d02d9c0f238b4906de69`; accepted exact quote/reservation setini stable typed BuildOrderId, WorkTicketId ve OperationId ile tek immutable job/workbench handoff'una bağlar.
- BuildOrder ve WorkTicket exact customer/request/quote/managed claim/workbench ile on line/ProductId/ItemInstanceId/ReservationId identity'yi dondurur. Bir quote, claim veya workbench ikinci active owner kabul etmez.
- Inventory tam managed seti commit öncesi exact yeniden doğrular ve tek terminal operation-keyed allocation receipt'i exactly-once yayınlar. Reservations/items canlı, yerinde ve unchanged kalır; generic checkout release/consume kullanılmaz. Exact replay revision drift üretmez; interrupted publication yalnız exact stored allocation receipt'ten recover edilir.
- Foreign/forged/stale/duplicate/missing/extra/drift/capacity/value-equal identity failures no-mutation fail-closed'dur. CustomPcQuoteAuthority ve AssemblyBuildAuthority izoledir; handoff assembly revision, receipt, slot, retained part veya cable state değiştirmez.
- GarageGraybox r34 canonical workbench'te dedicated interaction target ve collider-safe physical ticket taşır. Ticket job identity, exact `10/10` set ve `MONTAJA HAZIR • HENÜZ BAŞLAMADI` gösterir. Range/focus/LOS/empty-hands/fresh Interact, pause/co-edge/hold-release-repress, competing-target ve gerçek keyboard/mouse/gamepad customer→workbench route testlidir.
- Kanıt: EditMode `661/661`, PlayMode `66/66`; Universal Mac `329478891` bayt ve Apple M1/Metal exact r34 smoke. Exact clean Windows x64 IL2CPP/only-D3D11 build `1328828053` report baytı üretmiş, Intel Iris Xe/Direct3D 11.0 feature level 11.1 interactive runtime host/r34/work-ticket markerlarını birer kez, forbidden markerını sıfır vermiştir. Current-source Interact ownership ve no-teleport matrisi iki platformdaki full regression öncesinde/sonrasında korunmuştur.
- Repository: technical-source Guard `32721069982` başarılıdır. Exact source/docs `4e1ef4322d9ef049e3aac915c611474f6bee92fd`, tree `4df76fb1b50da53bdee7e65cb64acf0e73a5c018`, Guard `32723213686`; local staging metadata `2dc67d2`, provenance/current pre-USB head `6752927` ve Guard `32724718603` başarılıdır. ADR-0043 ve tarihli Evidence exact receiptsi ve `/Users/cixanla/Developer/PCShopEmpire3D/TestResults/issue66-f8afd62` canonical `9/9` kaynağını taşır. Yerel immutable milestone incoming/final doğrulamalarında `906/906` manifest, `896/896` exact Git source, `9/9` evidence, `17.330.935` bayt ve `1514481a…4121` manifest ile geçti. Doğru external physical USB, backup kökü ve önceki Issue #62 zinciri yeniden doğrulandı; `.incoming-issue66-6752927` ilk tam readback sonrasında atomik final adına taşındı ve final ikinci tam readback'i de aynı `906/906`, `896/896`, `9/9`, `17.330.935` bayt ve `1514481a…4121` sonuçlarıyla geçti. Internal/sibling AppleDouble ve incoming residue `0`dır. Fiziksel metadata `a80e325`, Guard `32726202296`, acceptance `18/18`, Issue #66 `CLOSED/COMPLETED`, Roadmap `Done`; parent Epic #10 açık/In Progress kalır. PR #67 accepted checkpoint'in integration aracıdır.
- Sınır: physical component transfer/build-kit completion, component attachment/cabling change, electrical power-on, POST/BIOS/OS, benchmark/QA, packaging/delivery, final settlement, Save/Guardian, final art/audio/VFX/UI ve Steam ayrı dependent paketlerdir.

## Güncel uygulama kaydı — Issue #68 canonical motherboard physical build-kit handoff

- Epic #10'un sıradaki bounded child'ı Issue #68'dir. Feature chain `2a69436` + `b0d2a97`, current technical head `480874191ee2c950e046ab2aee8be92d61d79fe4`, tree `e229788741df4c456840d356633e2a4bc1702516`; accepted work order/ticket allocation'ındaki canonical motherboard'ı exact line/product/item/reservation tuple'ıyla çözer.
- Stable child operation ve immutable pickup/place receipt'leri immediate/delayed exact replay'de aynı sonucu revision drift olmadan döndürür. Foreign, value-equal, wrong-kind/line/item/reservation/order, stale/conflict/capacity ve forgery yolları bütün Inventory/BuildKit/WorkOrder/Assembly state'inde no-mutation fail-closed'dur.
- Inventory capacity-one managed BuildKit container'ını Assembly Workbench ve seat custody'sinden ayrı tutar. Dar work-order-allocation bridge'i exact source → ActorHands → BuildKit hareketini yaparken live reservation ve allocation receipt'i korur; generic raw transfer, world drop, stack, cart ve Assembly bypass'ları kapalıdır.
- Presentation domain-first'tür: world parent/pose/visibility/physics domain commit'ten önce değişmez. Aynı Unity component ve stable ItemId pickup, carry, rotation, preview, placement, rollback/recovery ve replay boyunca korunur. Ticket authoritative sonuçtan sonra exact `0/10 → 1/10` gösterir; diğer dokuz item/reservation, quote price ve Assembly state/receipts untouched kalır.
...
- Exact detached-clean `4808741/e2297887` validation clone Unity 6000.3.21f1 ile full EditMode `675/675`, PlayMode `73/73`; failed/skipped/inconclusive `0` verdi. Test sonrası yalnız iki generated editor-setting deltası dedicated clone içinde kaldırıldı ve exact HEAD/tree/clean readback tekrar geçti.
- Universal Mac Development/StrictMode report `329571495` bayt; signed Universal executable + Apple M1/Metal r35 exact runtime başarılıdır. Exact Windows x64 IL2CPP/only-D3D11 report `1327308678` bayt; Intel Iris Xe/feature level 11.1 interactive runtime host/readiness/success/shutdown `1/1/1/1`, forbidden `0`, graceful/task/player cleanup `ok` verdi.
- Repository: technical-source Guard `32744068996`; exact source/docs `374094ceda9f8f65991e3906c62e1e4ba768b134`, tree `65418d089bc88c9f3dd435b93536c754fd4fef41` ve Guard `32750065918` başarılıdır; draft PR #69 açıktır. ADR-0044 ve tarihli Evidence, exact hashes/bytes/commands ve `/Users/cixanla/Developer/PCShopEmpire3D/TestResults/issue68-4808741` task-cleanup receipt dahil canonical procedure-bound `14/14` evidence kaynağını taşır.
- Fiziksel checkpoint: collision-free local immutable incoming/final ile doğru external physical USB incoming/atomik final hedefleri dört tam verifier readback'inde aynı `929/929` payload, `914/914` exact Git source, `14/14` evidence, `18.882.211` bayt ve `6d59ddb9…112a9` manifest sonucunu verdi; AppleDouble/incoming residue `0`, USB güvenle eject edildi. Yalnız final metadata commit/Guard, acceptance `20/20`, Issue close, Roadmap Done ve PR merge henüz tamamlanmamıştır. Issue #68 açık/In Progress; parent Epic #10 açık/In Progress kalır. Diğer dokuz component transferi, 10/10 kit completion, job-specific assembly, electrical/POST/OS/benchmark, packaging/delivery, final settlement, Save/Guardian, final art ve Steam ayrı paketlerdir.
