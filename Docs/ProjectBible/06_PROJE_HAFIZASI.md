...
| D-170 | Tamamlanan geliştirme paketleri USB'ye her dosya kaydında değil, doğrulanmış milestone snapshotı olarak yazılır. | Kullanıcı onayı — uygulandı 15 Ağustos 2026 | `2026-08-15_STAGE_B_SMALL_BOX_STACKING` ve güncel `2026-08-15_STAGE_B_LOADED_TRANSPORT_CART` final tracked kaynak + küçük kanıt setlerini taşır; SHA-256 manifest/readback ve source checksum kapıları kullanılır. Güncel snapshot 396 source + 6 evidence, 403 manifest satırı ve `a9e1d8e5…0444c3` manifest hash'iyle geçti. `.git`, cache, build uygulaması ve credential dışarıda kalır. |
...
Paket ve Stage A kapsamı 11 Ağustos 2026'da onaylandı ve uygulandı. Yerel Unity teknik temeli, development buildleri, Git geçmişi ve USB kaynak snapshot'ı tamamlandı. Stage B Core, oynanabilir GarageGraybox fiziksel akışı, tek-köşe görsel benchmark ve Unity bağımsız Catalog + Inventory authority ayrı testli paketlerde kapatıldı. Güncel baseline Edit Mode 161/161 ve Play Mode 14/14'tür; sıradaki bounded iş Issue #8 sipariş/teslimat/raf dilimidir.
