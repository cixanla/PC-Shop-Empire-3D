...
### Güncel checkpoint — Issue #87 r44 teknik/source-CI ve exact-head Mac/Windows native kapıları tamamlandı; source/docs, USB, insan ve idari kapanış sürüyor

- Technical head `25dc39ab02de93a416800acd17f53aacf83dca09`, tree `a736a764d0a52e950a4139002d6febc629df5987`. Canonical reserved PCIe/GPU cable yalnız exact `PowerCable` + `ModularPcie8PinPsuToGraphicsCard` role ve tam work-order/ticket/allocation line/product/item/reservation tuple'ıyla çözülür.
- PCIe/GPU cable, staged canonical motherboard+CPU+DDR5+M.2+processor-cooler+graphics-card+power-supply+ATX24+EPS12V sonrasında ayrı stable operation ve PCIe/GPU-specific managed capacity-one BuildKit üzerinden WorldFloor → ActorHands → PCIe/GPU BuildKit custody'sine taşınır. Decuple container claim all-or-none'dur; ilk dokuz slot/receipt/replay/revision/staged state korunur.
- GarageGraybox r44 ayrı tray/support/snap anchor, keyed `0° ↔ 180°` preview ve görünür `9/10 → 10/10` aggregate ekler. Active receipt primary/rotate/interact/drop input'unun tek sahibidir; Issue #63 PCIe/GPU installed route, #61/#62 cable routes, #59/#60 GPU/PSU ve diğer Assembly yolları aynı frame'i çalamaz.
- Full EditMode `709/709`, PlayMode `116/116`; failed/skipped/inconclusive `0`. Universal Mac report `330073048` bayt, valid deep/strict universal executable ve Apple M1/Metal exact r44 readiness + PCIe/GPU markerı başarılıdır. Technical Repository Guard `32921526334` exact source head üzerinde geçti.
- Complete bundle `7505067` bayt / `e055560…14ff`; collision-free detached-clean Windows exact head Unity 6000.3.21f1 x64 IL2CPP/only-D3D11 report `1339592274` bayt, `issue87-hardened-v1` fatal-token `0` verdi. Intel Iris Xe/Direct3D 11.0 feature level 11.1 interactive runtime exact host/readiness/success `1`, forbidden `0`, exit `0`, graceful shutdown, deleted task ve player/task residue `0` ile geçti.
- ADR-0053 ve tarihli Evidence exact source/tests/Mac/Windows/binary/procedure kanıtını bağlar. Marker açıkça `prerequisite-positioning=teleport-assisted` der; otomatik smoke exact-r44 gerçek insan oturumu yerine geçirilmez.
- Source/docs Repository Guard, canonical final `14/14` receipt, immutable local/healthy physical-USB çift readback, fiziksel metadata ve Issue #87 acceptance `25/25` henüz tamamlanmamıştır. Mevcut kesin durum `23/25` pass; insan ve fiziksel lifecycle kapıları bekler. Issue #87 ve draft PR #88 açık/In Progress, parent Epic #10 açık/In Progress kalır.
