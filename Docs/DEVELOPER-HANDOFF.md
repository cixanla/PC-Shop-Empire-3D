...
### Güncel checkpoint — Issue #85 r43 teknik ve exact-head Mac/Windows native kapıları tamamlandı; source/docs, USB, insan ve idari kapanış sürüyor

- Technical head `b6a74e932f4744b17388df7c7eb4d88f26d195f4`, tree `bd763ea0c8c6d2f5d256e467c4fca8b762ca4d84`. Canonical reserved EPS12V yalnız exact `PowerCable` + `ModularEps12v8PinPsuToMotherboard` role ve tam work-order/ticket/allocation line/product/item/reservation tuple'ıyla çözülür.
- EPS12V, staged canonical motherboard+CPU+DDR5+M.2+processor-cooler+graphics-card+power-supply+ATX24 sonrasında ayrı stable operation ve EPS12V-specific managed capacity-one BuildKit üzerinden WorldFloor → ActorHands → EPS12V BuildKit custody'sine taşınır. Nonuple container claim all-or-none'dur; ilk sekiz slot/receipt/replay/revision/staged state korunur.
- GarageGraybox r43 ayrı tray/support/snap anchor, keyed `0° ↔ 180°` preview ve görünür `8/10 → 9/10` aggregate ekler. Active receipt primary/rotate/interact/drop input'unun tek sahibidir; Issue #62 EPS12V installed route, #61/#63 cable routes, #60 PSU bay ve diğer Assembly yolları aynı frame'i çalamaz.
- Full EditMode `705/705`, PlayMode `115/115`; failed/skipped/inconclusive `0`. Universal Mac report `330018708` bayt, valid deep/strict universal executable ve Apple M1/Metal exact r43 readiness + EPS12V markerı başarılıdır.
- Complete bundle `7426223` bayt / `b2588f1…9559`; detached-clean Windows exact head Unity 6000.3.21f1 x64 IL2CPP/only-D3D11 report `1338310618` bayt, `issue85-hardened-v1` fatal-token `0` verdi. Intel Iris Xe/Direct3D 11.0 feature level 11.1 interactive runtime exact host/readiness/success `1`, forbidden `0`, exit `0`, graceful shutdown, deleted task ve player/task residue `0` ile geçti.
- ADR-0052 ve tarihli Evidence exact source/tests/Mac/Windows/binary/procedure kanıtını bağlar. Marker açıkça `prerequisite-positioning=teleport-assisted` der; otomatik smoke exact-r43 gerçek insan oturumu yerine geçirilmez.
- Source/docs Repository Guard, canonical final `14/14` receipt, immutable local/healthy physical-USB çift readback, fiziksel metadata ve Issue #85 acceptance `25/25` henüz tamamlanmamıştır. Mevcut kesin durum `22/25` pass; insan kapısı ve iki lifecycle kapısı bekler. Issue #85 açık/In Progress, parent Epic #10 açık/In Progress kalır.
